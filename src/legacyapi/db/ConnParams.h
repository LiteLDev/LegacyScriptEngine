...
public:
   ...
   * @brief Construct a new ConnParams object
   *
   * @param list  An initializer list like `{{"key1", "value1"}, {"key2",
   * "value2"}}`
   */
   ...
   * @note   If this object is constructed by `ConnParams(const std::string&
   * str)` or `ConnParams(const char* str)`, the return value is the same as the
   * parameter `str`. Otherwise, the return value will be empty.
   */
  std::string getRaw();
   ...
   * @param keys         The keys (If ignoreCase is true, keys must be
   * lowercase)
   ...
   */
  template <typename T>
  inline T get(const std::vector<std::string> &keys, bool ignoreCase = true,
               T defaultValue = T()) {
    Any value;
    int w = INT_MAX;
    for (auto &[k, v] : *this) {
