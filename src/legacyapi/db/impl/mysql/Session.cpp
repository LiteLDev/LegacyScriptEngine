...
MySQLSession::MySQLSession() {
  IF_ENDBG dbLogger.info("MySQLSession::MySQLSession: Constructed! this: {}",
                         (void *)this);
...
MySQLSession::~MySQLSession() {
  IF_ENDBG dbLogger.info("MySQLSession::MySQLSession: Destructor: this: {}",
                         (void *)this);
  close();
}
...
void MySQLSession::setSSL(const ConnParams &params) {
  auto p = params;
  auto key =
      p.get<std::string>({"sslkey", "ssl_key", "ssl-key"}); // Private key path
  auto cert = p.get<std::string>(
      {"sslcert", "ssl_cert", "ssl-cert"}); // Public certificate path
  auto ca = p.get<std::string>(
      {"sslca", "ssl_ca", "ssl-ca"}); // Certificate authority path
  auto capath =
      p.get<std::string>({"sslcapath", "ssl_capath",
                          "ssl-capath"}); // Certificate authority directory
  auto cipher = p.get<std::string>(
      {"sslcipher", "ssl_cipher", "ssl-cipher"}); // List of ciphers to use
  if (key.empty() || cert.empty()) {
    return;
  }
  mysql_ssl_set(
      conn, key.c_str(), cert.c_str(), (ca.empty() ? nullptr : ca.c_str()),
...
void MySQLSession::open(const ConnParams &params) {
  ...
  auto charset = p.get<std::string>(
      {"charset", "char_set", "characterset", "character_set", "charsetname",
  ...
  auto res =
      mysql_real_connect(conn, p.getHost().c_str(), p.getUsername().c_str(),
                         p.getPassword().c_str(),
                         (db.empty() ? nullptr : db.c_str()), port, nullptr, 0);
  if (!res) {
    throw std::runtime_error(
        "MySQLSession::MySQLSession: Failed to open database: " +
        std::string(mysql_error(conn)));
  }
  IF_ENDBG dbLogger.debug(
      "MySQLSession::open: Opened database: " + std::string(p.getHost()) + ":" +
...
bool MySQLSession::relogin(const std::string &user, const std::string &password,
                           const std::string &db) {
  IF_ENDBG dbLogger.debug(
      "MySQLSession::change: Changing user to {} and database to {}", user, db);
  auto res = mysql_change_user(conn, user.c_str(), password.c_str(),
                               (db.empty() ? nullptr : db.c_str()));
  return res == OK;
}

Session &MySQLSession::query(const std::string &query,
                             std::function<bool(const Row &)> callback) {
  IF_ENDBG dbLogger.debug("MySQLSession::query: Querying > " + query);
  auto res = mysql_query(conn, query.c_str());
  if (res != OK) {
    throw std::runtime_error("MySQLSession::query: Failed to query database: " +
                             std::string(mysql_error(conn)));
  }
  auto result = mysql_store_result(conn);
  if (!result) {
    // throw std::runtime_error("MySQLSession::query: Failed to store result: "
    // + std::string(mysql_error(conn)));
  ...
  IF_ENDBG dbLogger.debug(
      "MySQLSession::query: Query returned {} rows and {} fields", numRows,
      numFields);
  // Fetch column names
  RowHeader header;
...
uint64_t MySQLSession::getLastInsertId() const { return mysql_insert_id(conn); }
...
bool MySQLSession::isOpen() { return mysql_ping(conn) == OK; }
