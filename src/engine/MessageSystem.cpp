...
void ModuleMessage::handle(
    utils::Message &engineMsg) // Warning: Execute in another thread
{
  ModuleMessage msg;
...
utils::Message PackEngineMessage(string toModuleType,
...
utils::Message PackEngineMessage(string toModuleType, int messageId,
                                 ModuleMessage::MessageType type, string data) {
  auto &callbacks = globalShareData->messageSystemHandlers[toModuleType];
...
ModuleMessageResult ModuleMessage::broadcastLocal(MessageType type, string data,
                                                  int64_t delay) {
  ...
  for (auto &engine : engines) {
    try {
      ...
      logger.error("Fail to post message to plugin {}",
...
ModuleMessageResult ModuleMessage::broadcastGlobal(MessageType type,
                                                   string data, int64_t delay) {
  std::vector<ScriptEngine *> engineList;
  int msgId = -1;
  ...
  for (auto &engine : engines) {
    try {
      engine->messageQueue()->postMessage(
          PackEngineMessage(EngineManager::getEngineType(engine), type, data,
                            &msgId),
          std::chrono::milliseconds(delay));
      engineList.push_back(engine);
    } catch (const Exception &e) {
...
ModuleMessageResult ModuleMessage::broadcastTo(std::string toModuleType,
                                               MessageType type, string data,
                                               int64_t delay) {
  std::vector<ScriptEngine *> engineList;
  int msgId = -1;
...
ModuleMessageResult ModuleMessage::sendTo(ScriptEngine *engine,
                                          MessageType type, std::string data,
                                          int64_t delay) {
  int msgId = -1;
...
ModuleMessageResult ModuleMessage::sendToRandom(std::string toModuleType,
                                                MessageType type,
                                                std::string data,
                                                int64_t delay) {
  int msgId = -1;
...
bool ModuleMessage::sendResult(MessageType type, std::string data,
                               int64_t delay) {
  int msgId = header->id;
...
ModuleMessageResult::ModuleMessageResult(int messageId,
                                         std::vector<ScriptEngine *> engineList)
    : msgId(messageId), engineList(engineList),
...
bool ModuleMessageResult::waitForResultCount(int targetCount, int maxWaitTime) {
  ...
  while (maxWaitTime < 0 ? true
                         : GetCurrentTimeStampMS() - fromTime <= maxWaitTime) {
  ...
  for (auto &engine : engineList) {
    ...
    engine->messageQueue()->removeMessageIf([id](utils::Message &message) {
      return (GET_MESSAGE_HEADER(message))->id == id
                 ? utils::MessageQueue::RemoveMessagePredReturnType::
                       kRemoveAndContinue
                 : utils::MessageQueue::RemoveMessagePredReturnType::
                       kDontRemove;
    });
