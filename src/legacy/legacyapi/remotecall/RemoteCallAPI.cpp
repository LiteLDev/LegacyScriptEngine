...
void _onCallError(std::string const& msg, void* handle) {
    ...
    lse::getPluginManager().forEachPlugin([&handle, &plugin](std::string_view name, ll::plugin::Plugin& plu) {
        if (plu.getManifest().type == "native") {
            if (static_cast<ll::plugin::NativePlugin*>(&plu)->getHandle() == handle) {
                plugin = static_cast<ll::plugin::NativePlugin*>(&plu);
            }
        }
        return true;
    });
