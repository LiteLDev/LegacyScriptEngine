...
ClassDefine<EntityClass> EntityClassBuilder =
    defineClass<EntityClass>("LLSE_Entity")
        ...
        .instanceProperty("isTouchingDamageBlock",
                          &EntityClass::isTouchingDamageBlock)
        ...
        .instanceFunction("setUnderwaterMovementSpeed",
                          &EntityClass::setUnderwaterMovementSpeed)
        .instanceFunction("setLavaMovementSpeed",
                          &EntityClass::setLavaMovementSpeed)
        ...
        .instanceFunction("getBlockStandingOn",
                          &EntityClass::getBlockStandingOn)
        ...
        .instanceFunction("getEntityFromViewVector",
                          &EntityClass::getEntityFromViewVector)
        .instanceFunction("getBlockFromViewVector",
                          &EntityClass::getBlockFromViewVector)
        ...
        .instanceFunction("quickEvalMolangScript",
                          &EntityClass::quickEvalMolangScript)
...
Actor *EntityClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<EntityClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<EntityClass>(v)
        ->get();
  else
    return nullptr;
}
  ...
  try {
    ...
    if (!entity)
      ...
      return String::newString(
          std::to_string(entity->getOrCreateUniqueID().id));
  }
...
Local<Value> EntityClass::setPosDelta(const Arguments &args) {
  ...
  try {
    ...
    if (args.size() == 1) {
      ...
      delta = EngineScope::currentEngine()
                  ->getNativeInstance<FloatPos>(args[0])
                  ->getVec3();
    } else if (args.size() == 3) {
...
Local<Value> EntityClass::getFeetPos() {
  try {
    ...
    return FloatPos::newPos(entity->getFeetPosition(),
...
Local<Value> EntityClass::teleport(const Arguments &args) {
  ...
  try {
    ...
    return Boolean::newBoolean(
        entity->teleport(pos.getVec3(), pos.dim, pitch, yaw));
  }
...
Local<Value> EntityClass::hasContainer(const Arguments &args) {
  try {
    ...
    return Boolean::newBoolean(
        Level::hasContainer({pos.x, pos.y, pos.z}, entity->getDimensionId()));
  }
...
Local<Value> EntityClass::getContainer(const Arguments &args) {
  try {
    ...
    Vec3 pos = entity->getPosition();
    Container *container =
        Level::getContainer({pos.x, pos.y, pos.z}, entity->getDimensionId());
...
Local<Value> EntityClass::hurt(const Arguments &args) {
  ...
  try {
    ...
    if (args.size() == 2) {
      ...
      return Boolean::newBoolean(
          entity->hurtEntity(damage, (ActorDamageCause)type));
    }
    if (args.size() == 3) {
      CHECK_ARG_TYPE(args[1], ValueKind::kNumber);
      ...
      type = args[1].asNumber().toInt32();
      return Boolean::newBoolean(
          entity->hurtEntity(damage, (ActorDamageCause)type, source));
    }
    return Boolean::newBoolean(false);
  }
  CATCH("Fail in hurt!");
...
Local<Value> EntityClass::setHealth(const Arguments &args) {
  ...
  try {
    ...
    AttributeInstance *healthAttribute =
        entity->getMutableAttribute(Global<SharedAttributes>->HEALTH);
...
Local<Value> EntityClass::setAbsorption(const Arguments &args) {
  ...
  try {
    ...
    AttributeInstance *absorptionAttribute =
        entity->getMutableAttribute(Global<SharedAttributes>->ABSORPTION);
...
Local<Value> EntityClass::setAttackDamage(const Arguments &args) {
  ...
  try {
    ...
    AttributeInstance *attactDamageAttribute =
        entity->getMutableAttribute(Global<SharedAttributes>->ATTACK_DAMAGE);
...
Local<Value> EntityClass::setFollowRange(const Arguments &args) {
  ...
  try {
    ...
    AttributeInstance *followRangeAttribute =
        entity->getMutableAttribute(Global<SharedAttributes>->FOLLOW_RANGE);
...
Local<Value> EntityClass::setLuck(const Arguments &args) {
  ...
  try {
    ...
    AttributeInstance *luckAttribute =
        entity->getMutableAttribute(Global<SharedAttributes>->LUCK);
...
Local<Value> EntityClass::setLavaMovementSpeed(const Arguments &args) {
  ...
  try {
    ...
    AttributeInstance *lavaMovementSpeedAttribute = entity->getMutableAttribute(
...
Local<Value> EntityClass::setScale(const Arguments &args) {
  ...
  try {
    ...
    entity->getEntityData().set(ActorDataKeys::SCALE,
                                args[0].asNumber().toFloat());
    return Boolean::newBoolean(true);
  }
...
Local<Value> McClass::cloneMob(const Arguments &args) {
  ...
  try {
    ...
    if (args.size() == 2) {
      ...
      pos = {args[1].asNumber().toFloat(), args[2].asNumber().toFloat(),
             args[3].asNumber().toFloat(), args[4].toInt()};
    } else {
      LOG_WRONG_ARGS_COUNT();
      return Local<Value>();
    }
...
Local<Value> McClass::explode(const Arguments &args) {
  ...
  try {
    ...
    if (args.size() == 5) {
      ...
      CHECK_ARG_TYPE(args[2], ValueKind::kNumber);
      CHECK_ARG_TYPE(args[3], ValueKind::kNumber);
      pos = {args[0].asNumber().toFloat(), args[1].asNumber().toFloat(),
