...
SQLiteStmt::SQLiteStmt(sqlite3_stmt *stmt, const std::weak_ptr<Session> parent,
                       bool autoExecute)
    : Stmt(parent, autoExecute), stmt(stmt) {
  IF_ENDBG dbLogger.debug("SQLiteStmt::SQLiteStmt: Constructed! this: {}",
                          (void *)this);
...
int SQLiteStmt::getNextParamIndex() {
  ...
  IF_ENDBG dbLogger.debug(
      "SQLiteStmt::getNextParamIndex: The next param index is {}", result + 1);
  return result + 1;
}
...
void SQLiteStmt::fetchResultHeader() {
  ...
  for (int i = 0; i < colCnt; i++) {
    ...
    IF_ENDBG dbLogger.debug(
        "SQLiteStmt::fetchResultHeader: Column Name[{}]: {}", i, name);
    resultHeader->add(name);
  }
}

SQLiteStmt::~SQLiteStmt() {
  IF_ENDBG dbLogger.debug("SQLiteStmt::~SQLiteStmt: Destructor: this: {}",
                          (void *)this);
  close();
}
...
Stmt &SQLiteStmt::bind(const Any &value, int index) {
  ...
  if (getUnboundParams() == 0) {
    throw std::runtime_error(
        "SQLiteStmt::bind: All the parameters are already bound");
  }
  int res = SQLITE_OK;
  ...
  case Any::Type::DateTime:
    res = sqlite3_bind_text(stmt, index, value.get<std::string>().c_str(), -1,
                            SQLITE_TRANSIENT);
    break;
  case Any::Type::Blob:
    res =
        sqlite3_bind_blob(stmt, index, value.get<ByteArray>().data(),
                          (int)value.get<ByteArray>().size(), SQLITE_TRANSIENT);
    break;
  default:
  ...
  if (res != SQLITE_OK) {
    std::string e = fmt::format(
        "SQLiteStmt::bind: Failed to bind {} to parameter at index {}",
...
Stmt &SQLiteStmt::bind(const Any &value, const std::string &name) {
  ...
  for (auto &s : start) {
    ...
    if (i != 0) {
      if (index != 0) {
        throw std::runtime_error(
            "SQLiteStmt::bind: Multiple parameters match the name `" + name +
            "`");
      }
      index = i;
    }
  ...
  if (index == 0) {
    // Tips: The first parameter of SQLiteStmt::bind is the value to bind, the
    // second is the key!
    throw std::invalid_argument(
        "SQLiteStmt::bind: There isn't any statement parameter named `" + name +
        "`!");
  }
  IF_ENDBG dbLogger.debug("SQLiteStmt::bind: Parameter `{}` is at index {}",
                          name, index);
...
Row SQLiteStmt::_Fetch() {
  ...
  for (int i = 0; i < resultHeader->size(); i++) {
    ...
    case SQLITE_TEXT: {
      ...
      IF_ENDBG dbLogger.debug(
          "SQLiteStmt::_Fetch: Fetched TEXT type column: {} {}", i, text);
      row.push_back(text);
      break;
    }
    case SQLITE_BLOB: {
      ...
      IF_ENDBG {
        std::string out = "SQLiteStmt::_Fetch: Fetched BLOB type column: " +
                          std::to_string(i) + " ";
        for (auto &byte : arr) {
...
uint64_t SQLiteStmt::getAffectedRows() const { return affectedRowCount; }
...
SharedPointer<Stmt> SQLiteStmt::create(const std::weak_ptr<Session> &session,
