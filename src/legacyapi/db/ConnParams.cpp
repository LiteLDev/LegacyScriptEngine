    ...
    if (key) {
      if (item.is_string()) {
        throw std::invalid_argument(
            "ConnParams::ConnParams: Key must be string");
      }
...
std::string ConnParams::getHost() {
  return get<std::string>(
      {"host", "hostname", "server", "server_name", "ip", "address"}, true);
}

uint16_t ConnParams::getPort() {
  return get<uint16_t>({"port", "port_number", "port_num", "port_id", "portid"},
                       true, 0);
}

std::string ConnParams::getUsername() {
  return get<std::string>({"user", "username", "user_name", "usr", "usrname"},
                          true);
}
...
std::string ConnParams::getDatabase() {
  return get<std::string>({"database", "db", "db_name", "dbname", "path"},
                          true);
}
