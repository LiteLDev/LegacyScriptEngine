...
NativeFunction NativeFunction::getOrParse(const std::string& symbol) {
    ...
    if (funcAST->Signature->Quals & Q_Pointer64) result.mParams.push_back(NativeFunction::Types::Pointer);
...
Local<Value> ScriptNativeFunction::fromDescription(const Arguments& args) {
    ...
    result->mReturnVal =
        magic_enum::enum_cast<NativeFunction::Types>(args[0].toInt()).value_or(NativeFunction::Types::Void);
    for (size_t i = 1; i < argsSize; i++) {
        CHECK_ARG_TYPE(args[i], ValueKind::kNumber);
        result->mParams.push_back(
            magic_enum::enum_cast<NativeFunction::Types>(args[i].toInt()).value_or(NativeFunction::Types::Void)
        );
    }
    return scriptResult;
}
...
Local<Value> ScriptNativeFunction::fromScript(const Arguments& args) {
    ...
    CHECK_ARG_TYPE(args[0], ValueKind::kNumber);
    size_t argsSize = args.size();
    nativeScriptFunction->mReturnVal =
        magic_enum::enum_cast<NativeFunction::Types>(args[0].toInt()).value_or(NativeFunction::Types::Void);
    for (size_t i = 1; i < argsSize - 1; i++) {
        CHECK_ARG_TYPE(args[i], ValueKind::kNumber);
        nativeScriptFunction->mParams.push_back(
            magic_enum::enum_cast<NativeFunction::Types>(args[i].toInt()).value_or(NativeFunction::Types::Void)
        );
    }
