...
public:
  ...
  template <Types T> static Local<Value> getType() {
  ...
  static Local<Value> callNativeFunction(DCCallVM *vm,
                                         NativeFunction *funcSymbol,
                                         const Arguments &args);
  /* shared Hook Callback function that wrap script callback*/
  static char nativeCallbackHandler(DCCallback *cb, DCArgs *args,
                                    DCValue *result, void *userdata);

  // Cache Helper
  static Concurrency::concurrent_unordered_map<std::string, NativeFunction>
      parsedSymbol;
