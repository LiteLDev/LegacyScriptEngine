...
public:
  ...
  unsigned getId() { return header->id; }
  MessageType getType() { return type; }
  std::string getData() { return *data; }

  static ModuleMessageResult broadcastLocal(MessageType type, std::string data,
                                            int64_t delay = 0);
  static ModuleMessageResult broadcastGlobal(MessageType type, std::string data,
                                             int64_t delay = 0);
  static ModuleMessageResult broadcastTo(std::string toModuleType,
                                         MessageType type, std::string data,
                                         int64_t delay = 0);
  static ModuleMessageResult sendTo(ScriptEngine *engine, MessageType type,
                                    std::string data, int64_t delay = 0);
  static ModuleMessageResult sendToRandom(std::string toModuleType,
                                          MessageType type, std::string data,
                                          int64_t delay = 0);
