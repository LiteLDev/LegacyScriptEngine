...
NbtEndClass::NbtEndClass(const Local<Object> &scriptObj,
                         std::unique_ptr<EndTag> p)
...
EndTag *NbtEndClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtEndClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtEndClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtEndClass::pack(EndTag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtEndClass(Tag::asTypedTag<EndTag>(tag->copy())))
          ->getScriptObject();
  }
...
Local<Value> NbtEndClass::toString(const Arguments &args) {
  ...
  try {
    return String::newString(
        nbt->toJson(args.size() >= 1 ? args[0].toInt() : -1));
  }
...
NbtByteClass::NbtByteClass(const Local<Object> &scriptObj,
                           std::unique_ptr<ByteTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
ByteTag *NbtByteClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtByteClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtByteClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtByteClass::pack(ByteTag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtByteClass(Tag::asTypedTag<ByteTag>(tag->copy())))
          ->getScriptObject();
  }
...
NbtIntClass::NbtIntClass(const Local<Object> &scriptObj,
                         std::unique_ptr<IntTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
IntTag *NbtIntClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtIntClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtIntClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtIntClass::pack(IntTag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtIntClass(Tag::asTypedTag<IntTag>(tag->copy())))
          ->getScriptObject();
  }
...
NbtShortClass::NbtShortClass(const Local<Object> &scriptObj,
                             std::unique_ptr<ShortTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
ShortTag *NbtShortClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtShortClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtShortClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtShortClass::pack(ShortTag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtShortClass(Tag::asTypedTag<ShortTag>(tag->copy())))
          ->getScriptObject();
  }
...
NbtLongClass::NbtLongClass(const Local<Object> &scriptObj,
                           std::unique_ptr<Int64Tag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
Int64Tag *NbtLongClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtLongClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtLongClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtLongClass::pack(Int64Tag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtLongClass(Tag::asTypedTag<Int64Tag>(tag->copy())))
          ->getScriptObject();
  }
...
NbtFloatClass::NbtFloatClass(const Local<Object> &scriptObj,
                             std::unique_ptr<FloatTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
FloatTag *NbtFloatClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtFloatClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtFloatClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtFloatClass::pack(FloatTag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtFloatClass(Tag::asTypedTag<FloatTag>(tag->copy())))
          ->getScriptObject();
  }
...
NbtDoubleClass::NbtDoubleClass(const Local<Object> &scriptObj,
                               std::unique_ptr<DoubleTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
DoubleTag *NbtDoubleClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtDoubleClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtDoubleClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtDoubleClass::pack(DoubleTag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtDoubleClass(Tag::asTypedTag<DoubleTag>(tag->copy())))
          ->getScriptObject();
  }
...
NbtStringClass::NbtStringClass(const Local<Object> &scriptObj,
                               std::unique_ptr<StringTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
StringTag *NbtStringClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtStringClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtStringClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtStringClass::pack(StringTag *tag, bool noDelete) {
  try {
    if (noDelete) // unique_ptr 共享指针 + noDelete
    {
      ...
      return (new NbtStringClass(Tag::asTypedTag<StringTag>(tag->copy())))
          ->getScriptObject();
  }
...
NbtByteArrayClass::NbtByteArrayClass(const Local<Object> &scriptObj,
                                     std::unique_ptr<ByteArrayTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
NbtByteArrayClass *NbtByteArrayClass::constructor(const Arguments &args) {
  try {
    ...
    auto tag = ByteArrayTag((char *)buf.getRawBytes(), buf.byteLength());
...
ByteArrayTag *NbtByteArrayClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtByteArrayClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtByteArrayClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtByteArrayClass::set(const Arguments &args) {
  ...
  try {
    ...
    return Boolean::newBoolean(
        nbt->set(TagMemoryChunk((char *)buf.getRawBytes(), buf.byteLength())));
  }
  CATCH("Fail in NbtValueSet!")
}
...
NbtListClass::NbtListClass(const Local<Object> &scriptObj,
                           std::unique_ptr<ListTag> p)
    : ScriptClass(scriptObj) {
  this->nbt = std::move(p);
}
...
ListTag *NbtListClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtListClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtListClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else { list[index]->as_ptr<ByteTag>()->set(args[1].toInt());
    }
    return this->getScriptObject();
  }
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else { list[index]->as_ptr<IntTag>()->set(args[1].toInt());
    }
    return this->getScriptObject();
  }
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else { list[index]->as_ptr<ShortTag>()->set(args[1].toInt());
    }
    return this->getScriptObject();
  }
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else {
            list[index]->as_ptr<Int64Tag>()->set(args[1].asNumber().toInt64());
    }
    return this->getScriptObject();
  }
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else {
            list[index]->as_ptr<FloatTag>()->set(args[1].asNumber().toFloat());
    }
    return this->getScriptObject();
  }
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else {
            list[index]->as_ptr<DoubleTag>()->set(args[1].asNumber().toDouble());
    }
    return this->getScriptObject();
  }
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else { list[index]->as_ptr<StringTag>()->set(args[1].toStr());
    }
  ...
  try {
    ...
    if (index >= list.size() || index < 0) {
      LOG_ERROR_WITH_SCRIPT_INFO("Bad Index of NBT List!");
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Set wrong type of element into NBT
List!"); } else { auto data = args[1].asByteBuffer();
            list[index]->asByteArrayTag()->set(TagMemoryChunk((char*)data.getRawBytes(),
data.byteLength()));
    }
    return this->getScriptObject();
  }
  ...
  try {
    ...
    if (IsInstanceOf<NbtEndClass>(args[1])) {
      ...
      list[index] = NbtCompoundClass::extract(args[1])->clone().release();
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Unknown type! Cannot set Tag into
List"); return Local<Value>();
    }
    return this->getScriptObject();
  }
  ...
  try {
    if (IsInstanceOf<NbtEndClass>(args[0])) {
      ...
      nbt->add(NbtCompoundClass::extract(args[0])->clone());
            ...
            LOG_ERROR_WITH_SCRIPT_INFO("Unknown type! Cannot add Tag into
List"); return Local<Value>();
    }
    return this->getScriptObject();
  }
...
Local<Value> NbtListClass::getTag(const Arguments &args) {
  ...
  try {
    ...
    case Tag::Type::End:
      res = NbtEndClass::pack(list[index]->asEndTag(), true); // share
      ptr break;
    case Tag::Type::Byte:
      res = NbtByteClass::pack(list[index]->asByteTag(),
                               true); // share ptr break; case
      Tag::Type::Short
          : res = NbtShortClass::pack(list[index]->asShortTag(), true); //
      share ptr break;
    case Tag::Type::Int:
      res = NbtIntClass::pack(list[index]->asIntTag(),
                              true); // share ptr break; case
      Tag::Type::Int64
          : res = NbtLongClass::pack(list[index]->asInt64Tag(), true); //
      share ptr break;
    case Tag::Type::Float:
      res = NbtFloatClass::pack(list[index]->asFloatTag(),
                                true); // share ptr break; case
      Tag::Type::Double
          : res = NbtDoubleClass::pack(list[index]->asDoubleTag(), true);
      // share ptr break; case Tag::Type::String: res =
      NbtStringClass::pack(list[index]->asStringTag(),
                           true); // share ptr break; case
      Tag::Type::ByteArray
      ...
      break;
    case Tag::Type::List:
      res = NbtListClass::pack(list[index]->asListTag(), true); //
      share ptr break;
...
NbtCompoundClass::NbtCompoundClass(const Local<Object> &scriptObj,
                                   std::unique_ptr<CompoundTag> p)
    : ScriptClass(scriptObj) {
  nbt = std::move(p);
}
    ...
    for (int i = 0; i < keys.size(); ++i) {
      ...
      if (IsInstanceOf<NbtEndClass>(t))
        ...
        tag->put(keys[i], std::move(objTag));
                ...
                LOG_ERROR_WITH_SCRIPT_INFO("Wrong Type of data to set into NBT
Compound!");
      }
    }
  }
...
CompoundTag *NbtCompoundClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NbtCompoundClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NbtCompoundClass>(v)
        ->nbt.get();
  else
    return nullptr;
}
...
Local<Value> NbtCompoundClass::setDouble(const Arguments &args) {
  ...
  try {
    ...
    list.at(key).as_ptr<DoubleTag>()->value() = data;
    return this->getScriptObject();
  }
  CATCH("Fail in NBT SetDouble!");
}
...
Local<Value> NbtCompoundClass::getData(const Arguments &args) {
  ...
  try {
    ...
    return Tag2Value(list.at(key).as_ptr<Tag>());
  } catch (const out_of_range &e) {
    return Local<Value>();
  }
...
Local<Value> NbtCompoundClass::getTag(const Arguments &args) {
  ...
  try {
    ...
    Local<Value> res;
    switch (nbt->value().at(key).getId()) {
    case Tag::Type::End:
      res = NbtEndClass::pack(nbt->value().at(key).asEndTag(), true);
      // share ptr break; case Tag::Type::Byte: res =
      NbtByteClass::pack((ByteTag *)nbt->getByteTag(key),
                         true); // share ptr break;
    case Tag::Type::Short:
      res = NbtShortClass::pack(
          (ShortTag *)nbt->getShortTag(key),
          true); // share ptr break; case Tag::Type::Int: res =
      NbtIntClass::pack((IntTag *)nbt->getIntTag(key),
                        true); // share ptr break; case
      Tag::Type::Int64
          : res = NbtLongClass::pack(
                (Int64Tag *)nbt->getInt64Tag(key),
                true); // share ptr break; case Tag::Type::Float: res =
      NbtFloatClass::pack((FloatTag *)nbt->getFloatTag(key),
                          true); // share ptr break;
    case Tag::Type::Double:
      res = NbtDoubleClass::pack(
          (DoubleTag *)nbt->getDoubleTag(key),
          true); // share ptr break; case Tag::Type::String: res =
      NbtStringClass::pack((StringTag *)nbt->getStringTag(key),
                           true); // share ptr
      break;
...
Local<Value> NbtCompoundClass::toObject(const Arguments &args) {
  try {
    ...
    for (auto &[k, v] : comp) {
      obj.set(k, Tag2Value(v.as_ptr<Tag>(), true));
    }
    return obj;
  }
...
Local<Value> NbtStatic::parseBinaryNBT(const Arguments &args) {
  ...
  try {
    ...
    auto tag =
        CompoundTag::fromBinaryNBT(data.getRawBytes(), data.byteLength());
    if (tag)
      return NbtCompoundClass::pack(std::move(tag));
    else
      return Local<Value>();
...
bool IsNbtClass(Local<Value> value) {
  return IsInstanceOf<NbtEndClass>(value) ||
         IsInstanceOf<NbtByteClass>(value) ||
         IsInstanceOf<NbtShortClass>(value) ||
         IsInstanceOf<NbtIntClass>(value) ||
         IsInstanceOf<NbtLongClass>(value) ||
         IsInstanceOf<NbtFloatClass>(value) ||
         IsInstanceOf<NbtDoubleClass>(value) ||
         IsInstanceOf<NbtStringClass>(value) ||
         IsInstanceOf<NbtByteArrayClass>(value) ||
         IsInstanceOf<NbtListClass>(value) ||
         IsInstanceOf<NbtCompoundClass>(value);
}

//////////////////// Tag To Value ////////////////////

Local<Value> Tag2Value_CompoundHelper(CompoundTag *nbt,
...
Local<Value> Tag2Value_ListHelper(ListTag *nbt, bool autoExpansion = false) {
  ...
  auto &list = nbt->as_ptr<ListTag>()->value();
  for (auto &tag : list) {
    switch (tag->getId()) {
    case Tag::Type::End:
      res.add(Local<Value>());
      break;
    case Tag::Type::Byte:
      res.add(Number::newNumber(tag->as_ptr<ByteTag>()->value()));
      break;
    case Tag::Type::Short:
      res.add(Number::newNumber(tag->as_ptr<ShortTag>()->value()));
      break;
    case Tag::Type::Int:
      res.add(Number::newNumber(tag->as_ptr<IntTag>()->value()));
      break;
    case Tag::Type::Int64:
      res.add(Number::newNumber(tag->as_ptr<Int64Tag>()->value()));
      break;
    case Tag::Type::Float:
      res.add(Number::newNumber(tag->as_ptr<FloatTag>()->value()));
      break;
    case Tag::Type::Double:
      res.add(Number::newNumber(tag->as_ptr<DoubleTag>()->value()));
      break;
    case Tag::Type::String:
      res.add(String::newString(tag->as_ptr<StringTag>()->value()));
      break;
    case Tag::Type::ByteArray: {
      auto &data = tag->as_ptr<ByteArrayTag>()->value();
    ...
    case Tag::Type::List:
      if (!autoExpansion)
        ...
        res.add(Tag2Value_ListHelper(tag->as_ptr<ListTag>(), autoExpansion));
      break;
    case Tag::Type::Compound:
      if (!autoExpansion)
        res.add(NbtCompoundClass::pack(tag->asCompoundTag()));
      else
        res.add(Tag2Value_CompoundHelper(tag->as_ptr<CompoundTag>(),
                                         autoExpansion));
    default:
      res.add(Local<Value>());
      break;
...
Local<Value> Tag2Value_CompoundHelper(CompoundTag *nbt, bool autoExpansion) {
  ...
  auto &list = nbt->as_ptr<CompoundTag>()->value();
  for (auto &[key, tag] : list) {
    ...
    case Tag::Type::End:
      ...
      break;
    case Tag::Type::Byte:
      res.set(key, Number::newNumber(tag.as_ptr<ByteTag>()->get()));
      break;
    case Tag::Type::Short:
      res.set(key, Number::newNumber(tag.as_ptr<ShortTag>()->get()));
      break;
    case Tag::Type::Int:
      res.set(key, Number::newNumber(tag.as_ptr<IntTag>()->get()));
      break;
    case Tag::Type::Int64:
      res.set(key, Number::newNumber(tag.as_ptr<Int64Tag>()->get()));
      break;
    case Tag::Type::Float:
      res.set(key, Number::newNumber(tag.as_ptr<FloatTag>()->get()));
      break;
    case Tag::Type::Double:
      res.set(key, Number::newNumber(tag.as_ptr<DoubleTag>()->get()));
      break;
    case Tag::Type::String:
      res.set(key, String::newString(tag.as_ptr<StringTag>()->get()));
      break;
    case Tag::Type::ByteArray: {
      auto &data = tag.as_ptr<ByteArrayTag>()->value();
    ...
    case Tag::Type::List:
      if (!autoExpansion)
        ...
        res.set(key,
                Tag2Value_ListHelper(tag.as_ptr<ListTag>(), autoExpansion));
      break;
    case Tag::Type::Compound:
      if (!autoExpansion)
        res.set(key, NbtCompoundClass::pack(tag.asCompoundTag()));
      else
        res.set(key, Tag2Value_CompoundHelper(tag.as_ptr<CompoundTag>(),
                                              autoExpansion));
    default:
      res.set(key, Local<Value>());
      break;
...
Local<Value> Tag2Value(Tag *nbt, bool autoExpansion) {
  ...
  case Tag::Type::End:
    ...
    break;
  case Tag::Type::Byte:
    value = Number::newNumber(nbt->as_ptr<ByteTag>()->get());
    break;
  case Tag::Type::Short:
    value = Number::newNumber(nbt->as_ptr<ShortTag>()->get());
    break;
  case Tag::Type::Int:
    value = Number::newNumber(nbt->as_ptr<IntTag>()->get());
    break;
  case Tag::Type::Int64:
    value = Number::newNumber(nbt->as_ptr<Int64Tag>()->get());
    break;
  case Tag::Type::Float:
    value = Number::newNumber(nbt->as_ptr<FloatTag>()->get());
    break;
  case Tag::Type::Double:
    value = Number::newNumber(nbt->as_ptr<DoubleTag>()->get());
    break;
  case Tag::Type::String:
    value = String::newString(nbt->as_ptr<StringTag>()->get());
    break;
  case Tag::Type::ByteArray: {
    auto &data = nbt->as_ptr<ByteArrayTag>()->value();
  ...
  case Tag::Type::List:
    if (!autoExpansion)
      ...
      value = Tag2Value_ListHelper(nbt->as_ptr<ListTag>(), autoExpansion);
    break;
  case Tag::Type::Compound:
    if (!autoExpansion)
      ...
      value =
          Tag2Value_CompoundHelper(nbt->as_ptr<CompoundTag>(), autoExpansion);
    break;
  default:
    value = Local<Value>();
    break;
