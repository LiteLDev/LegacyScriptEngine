...
Local<Value> PlayerClass::simulateDestroy(const Arguments &args) {
  try {
    ...
    ScriptModuleMinecraft::ScriptFacing face =
        (ScriptModuleMinecraft::ScriptFacing)0;
    if (IsInstanceOf<IntPos>(args[0])) {
      ...
      auto block =
          EngineScope::currentEngine()->getNativeInstance<BlockClass>(args[0]);
...
Local<Value> PlayerClass::simulateRespawn(const Arguments &args) {
  try {
    ...
    if (sp->simulateRespawn()) {
      get()->teleport(sp->getSpawnPosition().bottomCenter(),
                      sp->getSpawnDimension());
      return Boolean::newBoolean(true);
    } else {
...
Local<Value> PlayerClass::simulateLocalMove(const Arguments &args) {
  ...
  try {
    ...
    if (IsInstanceOf<IntPos>(args[0])) {
      ...
      CHECK_ARG_TYPE(args[2], ValueKind::kNumber);
      Vec3 pos =
          Vec3(args[0].asNumber().toFloat(), args[1].asNumber().toFloat(),
...
inline Local<Value> NavigateResultToObject(
    ScriptModuleMinecraft::ScriptNavigationResult const &res) {
  auto obj = Object::newObject();
  obj.set(String::newString("isFullPath"),
          Boolean::newBoolean(res.mIsFullPath));
  auto path = Array::newArray();
  for (auto &pos : res.mPath) {
    path.add(
        Array::newArray({Number::newNumber(pos.x), Number::newNumber(pos.y),
                         Number::newNumber(pos.z)}));
  }
    ...
    if (args[0].isArray()) {
      ...
      for (size_t index = 0; index < arr.size(); ++index) {
        if (IsInstanceOf<IntPos>(arr.get(index)))
          path.emplace_back(
              IntPos::extractPos(arr.get(index))->getBlockPos().center());
          ...
          path.emplace_back(posArr.get(0).asNumber().toFloat(),
  ...
  try {
    ...
    if (item)
      return Boolean::newBoolean(
          sp->simulateUseItemOnBlock(*item, bpos, face, relativePos));
    else
      return Boolean::newBoolean(
          sp->simulateUseItemInSlotOnBlock(slot, bpos, face, relativePos));
  }
  CATCH("Fail in " __FUNCTION__ "!")
};
