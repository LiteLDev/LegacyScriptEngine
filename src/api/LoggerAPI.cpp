...
string &StrReplace(string &str, const string &to_replaced,
...
void inline LogDataHelper(ll::Logger::OutputStream *outStream,
...
Local<Value> LoggerClass::setLogLevel(const Arguments &args) {
  ...
  try {
    ...
    conf->maxLogLevel = conf->logger.consoleLevel = conf->logger.fileLevel =
        conf->logger.playerLevel = args[0].toInt();
    return Boolean::newBoolean(true);
  }
