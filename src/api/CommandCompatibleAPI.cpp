...
void LLSERegisterNewCmd(bool isPlayerCmd, string cmd, const string &describe,
  ...
  if (isPlayerCmd) {
    localShareData->playerCmdCallbacks[cmd] = {
        EngineScope::currentEngine(), level, script::Global<Function>(func)};
...
bool LLSERemoveCmdRegister(ScriptEngine *engine) {
  erase_if(localShareData->playerCmdCallbacks,
           [&engine](auto &data) { return data.second.fromEngine == engine; });
  erase_if(localShareData->consoleCmdCallbacks,
           [&engine](auto &data) { return data.second.fromEngine == engine; });
  return true;
}
// Helper
...
string LLSEFindCmdReg(bool isPlayerCmd, const string &cmd,
                      vector<string> &receiveParas, bool *fromOtherEngine) {
  std::unordered_map<std::string, std::string> &registeredMap =
      isPlayerCmd ? globalShareData->playerRegisteredCmd
                  : globalShareData->consoleRegisteredCmd;
  ...
  std::map<std::string, CmdCallbackData, CmdCallbackMapCmp> &cmdMap =
      isPlayerCmd ? localShareData->playerCmdCallbacks
                  : localShareData->consoleCmdCallbacks;
...
bool CallPlayerCmdCallback(Player *player, const string &cmdPrefix,
                           const vector<string> &paras) {
...
bool CallServerCmdCallback(const string &cmdPrefix,
                           const vector<string> &paras) {
