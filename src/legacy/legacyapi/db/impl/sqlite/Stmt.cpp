...
bool SQLiteStmt::step() {
    ...
    if (res == SQLITE_ROW) {
        ...
        IF_ENDBG lse::getSelfPluginInstance().getLogger().debug("SQLiteStmt::step: Successfully");
        stepped = true;
        return true;
    ...
    for (int i = 0; i < resultHeader->size(); i++) {
        ...
        case SQLITE_BLOB: {
            ...
            IF_ENDBG {
                ...
                lse::getSelfPluginInstance().getLogger().debug(out);
            }
            row.push_back(arr);
            break;
        }
...
Stmt& SQLiteStmt::reset() {
    ...
    IF_ENDBG lse::getSelfPluginInstance().getLogger().debug("SQLiteStmt::reexec: Reset successfully"
    );
    resultHeader.reset();
    steps            = 0;
    stepped          = false;
...
Stmt& SQLiteStmt::clear() {
    ...
    IF_ENDBG lse::getSelfPluginInstance().getLogger().debug("SQLiteStmt::clear: Reset successfully"
    );
...
SQLiteStmt::create(const std::weak_ptr<Session>& session, const std::string& sql, bool autoExecute) {
    ...
    if (raw->debugOutput)
        lse::getSelfPluginInstance().getLogger().debug("SQLiteStmt::create: Prepared > " + sql);
