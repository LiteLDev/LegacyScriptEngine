...
ClassDefine<ConfJsonClass> ConfJsonClassBuilder =
    defineClass<ConfJsonClass>("JsonConfigFile")
        ...
        .instanceFunction("reload",
                          selectOverloadedFunc<Local<Value> (ConfJsonClass::*)(
                              const Arguments &)>(&ConfJsonClass::reload))
        .instanceFunction("close",
                          selectOverloadedFunc<Local<Value> (ConfJsonClass::*)(
                              const Arguments &)>(&ConfJsonClass::close))
...
ClassDefine<ConfIniClass> ConfIniClassBuilder =
    defineClass<ConfIniClass>("IniConfigFile")
        ...
        .instanceFunction("reload",
                          selectOverloadedFunc<Local<Value> (ConfIniClass::*)(
                              const Arguments &)>(&ConfIniClass::reload))
        .instanceFunction("close",
                          selectOverloadedFunc<Local<Value> (ConfIniClass::*)(
                              const Arguments &)>(&ConfIniClass::close))
...
// 生成函数
ConfJsonClass::ConfJsonClass(const Local<Object> &scriptObj, const string &path,
                             const string &defContent)
...
// 生成函数
ConfIniClass::ConfIniClass(const Local<Object> &scriptObj, const string &path,
                           const string &defContent)
    : ScriptClass(scriptObj), ConfBaseClass(path) {
...
Local<Value> ConfIniClass::getStr(const Arguments &args) {
  ...
  try {
    ...
    return String::newString(
        iniConf->getString(args[0].toStr(), args[1].toStr(),
...
Local<Value> ConfIniClass::getInt(const Arguments &args) {
  ...
  try {
    ...
    return Number::newNumber(
        iniConf->getInt(args[0].toStr(), args[1].toStr(),
...
Local<Value> ConfJsonClass::newConf(const string &path,
                                    const string &defContent) {
  auto newp = new ConfJsonClass(path, defContent);
  return newp->getScriptObject();
}

Local<Value> ConfIniClass::newConf(const string &path,
                                   const string &defContent) {
