...
std::unique_ptr<KVDB> KVDB::create(const std::string &path, bool read_cache,
                                   int cache_sz, int Bfilter_bit) {
  CreateDirs(path);
  auto db = std::make_unique<KVDB>();
  db->_init(path.c_str(), true, read_cache, cache_sz, Bfilter_bit);
  return db;
}
std::unique_ptr<KVDB> KVDB::open(const std::string &path, bool create,
                                 bool read_cache, int cache_sz,
                                 int Bfilter_bit) {
  CreateDirs(path);
  auto db = std::make_unique<KVDB>();
...
void KVDB::_init(const char *path, bool create, bool read_cache, int cache_sz,
                 int Bfilter_bit) {
  ...
  if (!status.ok()) {
    ...
    levelDBLogger.error("{}", ll::utils::string_utils::tou8str(output));
  }
}
...
void KVDB::iter(
    std::function<bool(std::string_view key, std::string_view val)> const &fn) {
  leveldb::Iterator *it = db->NewIterator(rdopt);
  for (it->SeekToFirst(); it->Valid(); it->Next()) {
    auto k = it->key();
    auto v = it->value();
