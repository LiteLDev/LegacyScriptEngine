...
// throw exception
bool PluginManager::loadPlugin(const std::string &fileOrDirPath, bool isHotLoad,
                               bool mustBeCurrentModule) {
  ...
  string pluginFileName =
      RemoveRealAllExtension(UTF82String(p.filename().stem().u8string()));
  ...
  if (backendType == "PluginPackage") {
    ...
    auto [exitCode, output] = UncompressFile(
        fileOrDirPath, uncompressToDir, LLSE_PLUGIN_PACKAGE_UNCOMPRESS_TIMEOUT);
    if (exitCode != 0) {
  ...
  try {
    ...
    if (!PluginManager::getPlugin(pluginName))
      PluginManager::registerPlugin(realPath, pluginName, pluginName,
                                    ll::Version(1, 0, 0), {});
...
// Load plugin package
// This function must be called in correct backend
bool PluginManager::loadPluginPackage(const std::string &dirPath,
...
std::unordered_map<std::string, ll::Plugin *>
...
bool PluginManager::registerPlugin(std::string filePath, std::string name,
                                   std::string desc, ll::Version version,
    ...
    for (int i = 0; i < identifiers.size(); ++i)
      if (!identifiers.empty()) {
        ...
        if (id.find('*') != std::string::npos) {
          ...
          if (std::find(filesExts.begin(), filesExts.end(), compareExt) !=
              filesExts.end()) {
            // match
