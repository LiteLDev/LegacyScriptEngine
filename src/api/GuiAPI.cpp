...
ll::form::SimpleForm *SimpleFormClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<SimpleFormClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<SimpleFormClass>(v)
        ->get();
  else
    return nullptr;
}

void SimpleFormClass::sendForm(ll::form::SimpleForm *form, Player *player,
  ...
  form->sendTo(*player, [engine{EngineScope::currentEngine()},
                         callback{std::move(callbackFunc)}](Player &pl,
                                                            int chosen) {
...
ll::form::CustomForm *CustomFormClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<CustomFormClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<CustomFormClass>(v)
        ->get();
  else
    return nullptr;
}
...
// 成员函数
void CustomFormClass::sendForm(ll::form::CustomForm *form, Player *player,
                               script::Local<Function> &callback) {
  script::Global<Function> callbackFunc{callback};
  ...
  form->sendTo(
      *player,
      [engine{EngineScope::currentEngine()}, callback{std::move(callbackFunc)}](
          Player &pl, ll::form::CustomFormResult const &data) {
        if ((ll::getServerStatus() != ll::ServerStatus::Running))
          return;
        if (!EngineManager::isValid(engine))
          return;
...
Local<Value> CustomFormClass::addSlider(const Arguments &args) {
  ...
  try {
    ...
    form.appendSlider(args[0].toStr(), args[0].toStr(), minValue, maxValue,
                      step, defValue);
    return this->getScriptObject();
  }
