...
NativeStdString::NativeStdString(const Local<Object> &scriptObj,
                                 std::string *ptr)
...
std::string *NativeStdString::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NativeStdString>(v))
    return (std::string *)(EngineScope::currentEngine()
                               ->getNativeInstance<NativeStdString>(v)
                               ->get());
  else
    return nullptr;
}
...
NativeStdString *NativeStdString::constructor(const Arguments &args) {
  ...
  try {
    if (args[0].getKind() == ValueKind::kString) {
      return new NativeStdString(args.thiz(),
                                 new std::string(std::move(args[0].toStr())));
    }
    if (args[0].getKind() == ValueKind::kObject) {
      std::string *strPointer =
          static_cast<std::string *>(NativePointer::extract(args[0]));
  ...
  try {
    if (args.size() >= 2) {
      ...
      return Number::newNumber(
          (int64_t)mStr->find(args[0].toStr(), args[1].toInt()));
    } else
...
Local<Value> NativeStdString::rfind(const Arguments &args) {
  ...
  try {
    if (args.size() >= 2) {
      CHECK_ARG_TYPE(args[1], ValueKind::kNumber);
      return Number::newNumber(
          (int64_t)mStr->rfind(args[0].toStr(), args[1].toInt()));
    } else
