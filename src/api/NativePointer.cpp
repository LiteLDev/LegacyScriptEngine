...
//////////////////// NativePointer ////////////////////
ClassDefine<NativePointer> NativePointerBuilder =
    defineClass<NativePointer>("NativePointer")
        ...
        .instanceProperty("byte", &NativePointer::getMemByte,
                          &NativePointer::setMemByte)
        .instanceProperty("int8", &NativePointer::getChar,
                          &NativePointer::setChar)
        .instanceProperty("uint8", &NativePointer::getUchar,
                          &NativePointer::setUchar)
        .instanceProperty("int16", &NativePointer::getShort,
                          &NativePointer::setShort)
        .instanceProperty("uint16", &NativePointer::getUshort,
                          &NativePointer::setUshort)
        .instanceProperty("int32", &NativePointer::getInt,
                          &NativePointer::setInt)
        .instanceProperty("uint32", &NativePointer::getUint,
                          &NativePointer::setUint)
        .instanceProperty("long", &NativePointer::getLong,
                          &NativePointer::setLong)
        .instanceProperty("ulong", &NativePointer::getUlong,
                          &NativePointer::setUlong)
        .instanceProperty("int64", &NativePointer::getLonglong,
                          &NativePointer::setLonglong)
        .instanceProperty("uint64", &NativePointer::getUlonglong,
                          &NativePointer::setUlonglong)
        .instanceProperty("float", &NativePointer::getFloat,
                          &NativePointer::setFloat)
        .instanceProperty("double", &NativePointer::getDouble,
                          &NativePointer::setDouble)
        .instanceProperty("string", &NativePointer::getString,
                          &NativePointer::setString)
        .instanceProperty("bool", &NativePointer::getBool,
                          &NativePointer::setBool)
...
Local<Value> NativePointer::fromAddress(const Arguments &args) {
  ...
  if (args[0].isString()) {
    std::stringstream ss;
    ss << std::hex << args[0].asString().toString();
    uintptr_t res;
    ss >> res;
...
Local<Value> NativePointer::fromSymbol(const Arguments &args) {
  ...
  return newNativePointer(
      ll::memory::resolveSymbol(args[0].asString().toStringHolder().c_str()));
}
...
void *NativePointer::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<NativePointer>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<NativePointer>(v)
        ->get();
  else
    return nullptr;
}
...
Local<Value> NativePointer::offset(const Arguments &args) {
  ...
  try {
    ...
    if (!ptr)
      return Local<Value>();
    return newNativePointer(
        (void *)((uintptr_t)ptr + args[0].asNumber().toInt32()));
  }
...
void NativePointer::setUlonglong(const Local<Value> &value) {
  try {
    *(unsigned long long *)get() =
        (unsigned long long)value.asNumber().toInt64();
  } catch (...) {
