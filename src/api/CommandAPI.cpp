...
ClassDefine<void> PermissionStaticBuilder =
    EnumDefineBuilder<CommandPermissionLevel>::build("PermType");
ClassDefine<void> ParamTypeStaticBuilder =
    EnumDefineBuilder<DynamicCommand::ParameterType>::build("ParamType");
ClassDefine<void> ParamOptionStaticBuilder =
    EnumDefineBuilder<CommandParameterOption>::build("ParamOption");
...
ClassDefine<CommandClass> CommandClassBuilder =
    defineClass<CommandClass>("LLSE_Command")
        ...
        .instanceFunction("removeSoftEnumValues",
                          &CommandClass::removeSoftEnumValues)
...
bool LLSERemoveCmdCallback(script::ScriptEngine *engine) {
  erase_if(localShareData->commandCallbacks,
           [&engine](auto &data) { return data.second.fromEngine == engine; });
  return true;
}
...
Local<Value> convertResult(DynamicCommand::Result const &result) {
  ...
  case DynamicCommand::ParameterType::BlockPos: {
    ...
    return IntPos::newPos(result.get<BlockPos>(),
                          dim ? (int)dim->getDimensionId() : -1);
  }
  case DynamicCommand::ParameterType::Vec3: {
    auto dim = result.origin->getDimension();
    return FloatPos::newPos(result.get<Vec3>(),
                            dim ? (int)dim->getDimensionId() : -1);
  }
  ...
  case DynamicCommand::ParameterType::JsonValue:
    return String::newString(
        JsonHelpers::serialize(result.getRaw<Json::Value>()));
  case DynamicCommand::ParameterType::Item:
    return ItemClass::newItem(new ItemStack(
        result.getRaw<CommandItem>()
            .createInstance(1, 1, *new CommandOutput(CommandOutputType::None),
                            true)
  ...
  case DynamicCommand::ParameterType::Effect:
    return String::newString(
        result.getRaw<MobEffect const *>()->getResourceName());
  ...
  case DynamicCommand::ParameterType::Command:
    return String::newString(
        result.getRaw<std::unique_ptr<Command>>()->getCommandName());
  case DynamicCommand::ParameterType::ActorType:
    return String::newString(
        result.getRaw<ActorDefinitionIdentifier const *>()->getCanonicalName());
  default:
    return Local<Value>(); // null
    break;
  }
...
Local<Value> McClass::newCommand(const Arguments &args) {
  ...
  try {
    ...
    if (instance) {
      logger.info("Dynamic command {} already exists, changes will not be "
                  "applied except for setOverload!",
                  name);
      return CommandClass::newCommand(
          const_cast<std::add_pointer_t<
              std::remove_cv_t<std::remove_pointer_t<decltype(instance)>>>>(
              instance));
    }
...
CommandClass::CommandClass(std::unique_ptr<DynamicCommandInstance> &&p)
    : ScriptClass(ScriptClass::ConstructFromCpp<CommandClass>{}),
      uptr(std::move(p)), ptr(uptr.get()), registered(false){};

CommandClass::CommandClass(DynamicCommandInstance *p)
    : ScriptClass(ScriptClass::ConstructFromCpp<CommandClass>{}), uptr(),
      ptr(p), registered(true){};

Local<Object>
CommandClass::newCommand(std::unique_ptr<DynamicCommandInstance> &&p) {
...
Local<Value> CommandClass::newParameter(const Arguments &args) {
  ...
  try {
    ...
    DynamicCommand::ParameterType type =
        parseEnum<DynamicCommand::ParameterType>(args[1]);
    ...
    return Number::newNumber((int64_t)get()
                                 ->newParameter(name, type, optional,
...
Local<Value> CommandClass::addOverload(const Arguments &args) {
  try {
    ...
    auto command = get();
    if (args.size() == 0)
      return Boolean::newBoolean(command->addOverload(
          std::vector<DynamicCommandInstance::ParameterIndex>{}));
    if (args[0].isNumber()) {
...
void onExecute(
  ...
  if (localShareData->commandCallbacks.find(commandName) ==
      localShareData->commandCallbacks.end()) {
    logger.warn("Command {} failed to execute, is the plugin unloaded?",
                commandName);
    return;
  }
  ...
  try {
    ...
    auto cmd = CommandClass::newCommand(
        const_cast<DynamicCommandInstance *>(instance));
    ...
    localShareData->commandCallbacks[commandName].func.get().call({}, cmd, ori,
                                                                  outp, args);
  }
...
Local<Value> CommandClass::setCallback(const Arguments &args) {
  ...
  try {
    ...
    localShareData->commandCallbacks[commandName] = {
        EngineScope::currentEngine(), 0, script::Global<Function>(func)};
    if (registered)
...
Local<Value> CommandClass::toString(const Arguments &args) {
  try {
    return String::newString(
        fmt::format("<Command({})>", get()->getCommandName()));
  }
...
Local<Value> CommandClass::addSoftEnumValues(const Arguments &args) {
  ...
  try {
    ...
    auto enums = parseStringList(args[1].asArray());
    return Boolean::newBoolean(
        get()->addSoftEnumValues(name, std::move(enums)));
  }
...
Local<Value> CommandClass::removeSoftEnumValues(const Arguments &args) {
  ...
  try {
    ...
    auto enums = parseStringList(args[1].asArray());
    return Boolean::newBoolean(
        get()->removeSoftEnumValues(name, std::move(enums)));
  }
