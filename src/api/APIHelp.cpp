...
template <typename T> void PrintValue(T &out, Local<Value> v) {
  ...
  case ValueKind::kObject: {
    ...
    if (intpos != nullptr) {
      out << DimId2Name(intpos->dim) << "(" << intpos->x << "," << intpos->y
          << "," << intpos->z << ")";
      break;
    }
    ...
    if (IsInstanceOf<NativePointer>(v)) {
      std::stringstream ss;
      ss << std::hex
         << (intptr_t)EngineScope::currentEngine()
    ...
    if (IsInstanceOf<ScriptNativeFunction>(v)) {
      std::stringstream ss;
      ScriptNativeFunction *func =
          EngineScope::currentEngine()->getNativeInstance<ScriptNativeFunction>(
              v);
