        ...
        .property("Open",
                  [] {
                    return Number::newNumber(
                        (int)WebSocketClient::Status::Open);
                  })
        .property("Closing",
                  [] {
                    return Number::newNumber(
                        (int)WebSocketClient::Status::Closing);
                  })
        .property("Closed",
                  [] {
                    return Number::newNumber(
                        (int)WebSocketClient::Status::Closed);
                  })
        .build();
...
ClassDefine<HttpResponseClass> HttpResponseClassBuilder =
    defineClass<HttpResponseClass>("HttpResponse")
        ...
        .instanceProperty("headers", &HttpResponseClass::getHeaders,
                          &HttpResponseClass::setHeaders)
        .instanceProperty("status", &HttpResponseClass::getStatus,
                          &HttpResponseClass::setStatus)
        .instanceProperty("body", &HttpResponseClass::getBody,
                          &HttpResponseClass::setBody)
        .instanceProperty("version", &HttpResponseClass::getVersion,
                          &HttpResponseClass::setVersion)
        .instanceProperty("reason", &HttpResponseClass::getReason,
                          &HttpResponseClass::setReason)
        .build();
...
void WSClientClass::initListeners() {
  ...
  ws->OnBinaryReceived([nowList{
                           &listeners[int(WSClientEvents::onBinaryReceived)]}](
    ...
    if (!nowList->empty())
      for (auto &listener : *nowList) {
        if (!EngineManager::isValid(listener.engine))
          return;
        EngineScope enter(listener.engine);
        NewTimeout(listener.func.get(),
                   {ByteBuffer::newByteBuffer(data.data(), data.size())}, 1);
      }
  });
  ...
  ws->OnLostConnection(
      [nowList{&listeners[int(WSClientEvents::onLostConnection)]}](
...
void WSClientClass::initListeners_s() {
  ...
  ws->OnBinaryReceived(
      [nowList{&listeners[int(WSClientEvents::onBinaryReceived)]},
       engine = EngineScope::currentEngine()](WebSocketClient &client,
                                              vector<uint8_t> data) {
        if ((ll::getServerStatus() != ll::ServerStatus::Running) ||
            !EngineManager::isValid(engine) || engine->isDestroying())
          return;
        std::thread([nowList, engine, data = std::move(data)]() mutable {
          ...
          if (!nowList->empty())
            for (auto &listener : *nowList) {
              listener.func.get().call(
                  {}, {ByteBuffer::newByteBuffer(data.data(), data.size())});
            }
        }).detach();
      });
...
Local<Value> WSClientClass::connect(const Arguments &args) {
  ...
  try {
    ...
    RecordOperation(ENGINE_OWN_DATA()->pluginName, "ConnectToWebsocketServer",
                    target);
    ws->Connect(target);
...
Local<Value> WSClientClass::connectAsync(const Arguments &args) {
  ...
  try {
    ...
    std::thread([ws{this->ws}, target, callback{std::move(callbackFunc)},
                 engine{EngineScope::currentEngine()},
...
Local<Value> WSClientClass::send(const Arguments &args) {
  ...
  try {
    if (args[0].isString())
      ...
      ws->SendBinary((char *)args[0].asByteBuffer().getRawBytes(),
...
#define ADD_CALLBACK(method, path, func)                                       \
  callbacks.emplace(                                                           \
      make_pair(path, HttpServerCallback{EngineScope::currentEngine(),         \
                                         script::Global<Function>{func},       \
                                         HttpRequestType::method, path}));     \
  svr->##method##(path.c_str(), [this, engine = EngineScope::currentEngine()]( \
...
HttpServerClass::HttpServerClass(const Local<Object> &scriptObj)
    : ScriptClass(scriptObj), svr(new Server) {}
...
Local<Value> HttpServerClass::onPreRouting(const Arguments &args) {
  ...
  try {
    preRoutingCallback = {EngineScope::currentEngine(),
                          script::Global{args[0].asFunction()}};
    svr->set_pre_routing_handler([this, engine = EngineScope::currentEngine()](
                                     const Request &req, Response &resp) {
      if ((ll::getServerStatus() != ll::ServerStatus::Running) ||
          !EngineManager::isValid(engine) || engine->isDestroying())
      ...
      std::thread([this, engine, req, &resp, &handled] {
        ...
        auto respObj = new HttpResponseClass(resp);
        auto res =
            this->preRoutingCallback.func.get().call({}, reqObj, respObj);
        if (res.isBoolean() && res.asBoolean().value() == false) {
          handled = true;
        }
        resp = *respObj->get();
      }).join();
      return handled ? Server::HandlerResponse::Handled
                     : Server::HandlerResponse::Unhandled;
    });
    return this->getScriptObject();
  }
...
Local<Value> HttpServerClass::onPostRouting(const Arguments &args) {
  ...
  try {
    postRoutingCallback = {EngineScope::currentEngine(),
                           script::Global{args[0].asFunction()}};
    svr->set_post_routing_handler([this, engine = EngineScope::currentEngine()](
                                      const Request &req, Response &resp) {
      if ((ll::getServerStatus() != ll::ServerStatus::Running) ||
          !EngineManager::isValid(engine) || engine->isDestroying())
        return;
...
Local<Value> HttpServerClass::onError(const Arguments &args) {
  ...
  try {
    errorCallback = {EngineScope::currentEngine(),
                     script::Global{args[0].asFunction()}};
...
Local<Value> HttpServerClass::onException(const Arguments &args) {
  ...
  try {
    exceptionCallback = {EngineScope::currentEngine(),
                         script::Global{args[0].asFunction()}};
    svr->set_exception_handler(
        [this, engine = EngineScope::currentEngine()](
          ...
          std::thread([this, engine, req, &resp, e] {
            ...
            try {
              ...
              this->exceptionCallback.func.get().call(
                  {}, reqObj, respObj, String::newString(exp.what()));
            }
            resp = *respObj->get();
          }).join();
        });
...
Local<Value> HttpServerClass::listen(const Arguments &args) {
  ...
  try {
    ...
    RecordOperation(ENGINE_OWN_DATA()->pluginName, "StartHttpServer",
                    fmt::format("on {}:{}", addr, port));

    std::thread th(
        [this](string addr, int port) { svr->listen(addr.c_str(), port); },
        addr, port);
    th.detach();
...
HttpRequestClass::HttpRequestClass(const Local<Object> &scriptObj,
                                   const Request &req)
...
HttpResponseClass::HttpResponseClass(const Local<Object> &scriptObj,
                                     const Response &resp)
    ...
    for (auto &key : keys) {
      ...
      if (value.isArray()) {
        for (size_t i = 0ULL; i < value.asArray().size(); ++i) {
          resp->headers.insert(
              make_pair(key_str, ValueToString(value.asArray().get(i))));
        }
        return;
      }
...
void SplitHttpUrl(const std::string &url, string &host, string &path) {
  ...
  auto splitPos =
      host.find('/', foundProtocol ? host.find('/') + 2
                                   : 0); // 查找协议后的第一个/分割host与路径
...
bool HttpGetSync(const std::string &url, int *statusRtn, std::string *dataRtn,
                 int timeout = -1) {
  string host, path;
  SplitHttpUrl(url, host, path);
...
Local<Value> NetworkClass::httpGet(const Arguments &args) {
  ...
  try {
    ...
    auto lambda = [callback{std::move(callbackFunc)},
                   engine{EngineScope::currentEngine()}](int status,
                                                         string body) {
      if ((ll::getServerStatus() != ll::ServerStatus::Running) ||
          !EngineManager::isValid(engine) || engine->isDestroying())
        return;
      ...
      try {
        NewTimeout(callback.get(),
                   {Number::newNumber(status), String::newString(body)}, 1);
  ...
  try {
    ...
    if (args.size() > 4) {
      ...
      return Boolean::newBoolean(
          HttpPost(target, maps, args[2].toStr(), args[3].toStr(), lambda));
    }
    return Boolean::newBoolean(
        HttpPost(target, args[1].toStr(), args[2].toStr(), lambda));
  }
