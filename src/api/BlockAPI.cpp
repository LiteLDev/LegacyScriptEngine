...
ClassDefine<BlockClass> BlockClassBuilder =
    defineClass<BlockClass>("LLSE_Block")
        ...
        .instanceProperty("isWaterBlockingBlock",
                          &BlockClass::isWaterBlockingBlock)
...
// generating function
Local<Object> BlockClass::newBlock(Block const *p, BlockPos const *pos,
                                   int dim) {
...
Block *BlockClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<BlockClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<BlockClass>(v)
        ->get();
  else
    return nullptr;
}
