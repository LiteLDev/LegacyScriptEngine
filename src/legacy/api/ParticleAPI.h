...
public:
    explicit ParticleSpawner(const Local<Object>& scriptObj) : ScriptClass(scriptObj), ParticleCUI() {}

    static ParticleSpawner* create(const Arguments& args);

    Local<Value> getDisplayRadius() { return Number::newNumber(static_cast<long long>(displayRadius)); }
    Local<Value> getHighDetial() { return Boolean::newBoolean(highDetial); }
    Local<Value> getDoubleSide() { return Boolean::newBoolean(doubleSide); }
    void         setDisplayRadius(const Local<Value>& value) { displayRadius = value.asNumber().toInt64(); }
    void         setHighDetial(const Local<Value>& value) { highDetial = value.asBoolean().value(); }
    void         setDoubleSide(const Local<Value>& value) { doubleSide = value.asBoolean().value(); }
