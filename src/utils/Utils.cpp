...
unsigned long long GetCurrentTimeStampMS() {
  ...
  struct tm currTm = {currentTime.wSecond,    currentTime.wMinute,
                      currentTime.wHour,      currentTime.wDay,
                      currentTime.wMonth - 1, currentTime.wYear - 1900};
...
std::string Raw_RandomGuid() {
  ...
  char dst[36] = {0};
  snprintf(dst, 36, "%08x%04x%04x%02x%02x%02x%02x%02x%02x%02x%02x", guid.Data1,
...
std::wstring Raw_RandomGuidW() {
  ...
  wchar_t dst[36] = {0};
  _snwprintf_s(dst, 36, L"%08x%04x%04x%02x%02x%02x%02x%02x%02x%02x%02x",
