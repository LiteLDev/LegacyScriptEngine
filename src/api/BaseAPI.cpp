...
ClassDefine<void> DamageCauseEnumBuilder =
    EnumDefineBuilder<ActorDamageCause>::build("DamageCause");
...
ClassDefine<DirectionAngle> DirectionAngleBuilder =
    defineClass<DirectionAngle>("DirectionAngle")
        ...
        .instanceProperty("pitch", &DirectionAngle::getPitch,
                          &DirectionAngle::setPitch)
        .instanceProperty("yaw", &DirectionAngle::getYaw,
                          &DirectionAngle::setYaw)
...
Local<Value> IntPos::toString() {
  try {
    return String::newString(
        fmt::format("{}({}, {}, {})", DimId2Name(dim), x, y, z));
  }
...
Local<Object> DirectionAngle::newAngle(float pitch, float yaw) {
  return EngineScope::currentEngine()->newNativeClass<DirectionAngle>(pitch,
                                                                      yaw);
}
...
Local<Value> McClass::newIntPos(const Arguments &args) {
  ...
  try {
    return IntPos::newPos(args[0].toInt(), args[1].toInt(), args[2].toInt(),
                          args[3].toInt());
  }
...
Local<Value> McClass::newFloatPos(const Arguments &args) {
  ...
  try {
    return FloatPos::newPos(args[0].asNumber().toFloat(),
