...
bool Session::relogin(const std::string &user, const std::string &password,
                      const std::string &db) {
...
ResultSet Session::query(const std::string &query) {
  ...
  IF_ENDBG {
    if (result.valid()) {
      ...
      for (auto &str :
           ll::string_utils::splitByPattern(result.toTableString(), "\n")) {
        dbLogger.debug(str);
      }
    } else {
...
SharedPointer<Session> Session::create(const ConnParams &params) {
  ...
  if (names.count(scheme)) {
    ...
    throw std::runtime_error(
        "Session::create: Unknown/Unsupported database type");
  }
}
SharedPointer<Session> Session::create(DBType type, const ConnParams &params) {
  return _Create(type, params);
}
SharedPointer<Session> Session::create(DBType type, const std::string &host,
                                       uint16_t port, const std::string &user,
                                       const std::string &password,
                                       const std::string &database) {
  return _Create(type, {{"host", host},
                        {"port", port},
                        {"user", user},
                        {"password", password},
                        {"database", database}});
...
std::vector<std::weak_ptr<Session>> Session::sessionPool =
