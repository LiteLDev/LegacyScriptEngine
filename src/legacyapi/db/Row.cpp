...
void RowHeader::remove(const std::string &name) {
  ...
  throw std::runtime_error("RowHeader::remove: Column " + name +
                           " is not found");
}
...
Row::Row(const std::initializer_list<Any> &list,
         const std::shared_ptr<RowHeader> &header)
...
Row::Row(const Row &row) : header(row.header) { *this = row; }
...
void Row::forEach(
    std::function<bool(const std::string &, const Any &)> cb) const {
  if (!this->header)
    return;
  int i = 0;
  for (auto &col : *this->header) {
