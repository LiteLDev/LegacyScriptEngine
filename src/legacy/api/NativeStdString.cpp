...
NativeStdString::NativeStdString(const Local<Object>& scriptObj, std::string* ptr) : ScriptClass(scriptObj) {
    set(ptr);
}

NativeStdString::NativeStdString(std::string* ptr) : ScriptClass(ScriptClass::ConstructFromCpp<NativeStdString>{}) {
    set(ptr);
}

std::string* NativeStdString::extract(Local<Value> v) {
    if (EngineScope::currentEngine()->isInstanceOf<NativeStdString>(v))
        return (std::string*)(EngineScope::currentEngine()->getNativeInstance<NativeStdString>(v)->get());
    else return nullptr;
}
...
NativeStdString* NativeStdString::constructor(const Arguments& args) {
    ...
    try {
        if (args[0].getKind() == ValueKind::kString) {
            return new NativeStdString(args.thiz(), new std::string(std::move(args[0].toStr())));
        }
        if (args[0].getKind() == ValueKind::kObject) {
            std::string* strPointer = static_cast<std::string*>(NativePointer::extract(args[0]));
...
Local<Value> NativeStdString::front() {
    try {
        return String::newString(std::string(mStr->front(), 1));
    }
...
Local<Value> NativeStdString::back() {
    try {
        return String::newString(std::string(mStr->back(), 1));
    }
...
Local<Value> NativeStdString::at(const Arguments& args) {
    ...
    try {
        return String::newString(std::string(mStr->at(args[0].toInt()), 1));
...
Local<Value> NativeStdString::find(const Arguments& args) {
    ...
    try {
        ...
        } else return Number::newNumber((int64_t)mStr->find(args[0].toStr()));
    }
...
Local<Value> NativeStdString::rfind(const Arguments& args) {
    ...
    try {
        ...
        } else return Number::newNumber((int64_t)mStr->rfind(args[0].toStr()));
    }
...
Local<Value> NativeStdString::substr(const Arguments& args) {
    ...
    try {
        ...
        } else res = std::move(mStr->substr(args[0].toInt()));
        return newNativeStdString(new std::string(std::move(res)));
    }
...
Local<Value> NativeStdString::clone() {
    try {
        return NativeStdString::newNativeStdString(new std::string(*mStr));
    }
