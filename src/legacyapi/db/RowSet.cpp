...
RowSet::RowSet(const RowHeader &header)
    : std::vector<Row>(), header(new RowHeader(header)) {}
RowSet::RowSet(RowSet &&set) noexcept
    : std::vector<Row>(std::move(set)), header(set.header) {}
  ...
  for (auto &row : *this) {
    for (size_t i = 0; i < row.size(); ++i) {
      ...
      if (val.is_null()) {
        ...
        result +=
            fmt::format("| {:<{}} ", val.get<std::string>(), colWidths[i]);
      }
    }
    result += "|\n";
  }
