...
Local<Value> McClass::getStructure(const Arguments &args) {
  ...
  try {
    ...
    auto structure = StructureTemplate::create(
        "",
        ll::service::getLevel()
            ->getDimension(pos1->getDimensionId())
            ->getBlockSourceFromMainChunkSource(),
        BoundingBox(pos1->getBlockPos(), pos2->getBlockPos()), ignoreBlocks,
        ignoreEntities);
...
Local<Value> McClass::setStructure(const Arguments &args) {
  ...
  try {
    ...
    structure->placeInWorld(ll::service::getLevel()
                                ->getDimension(pos->getDimensionId())
                                ->getBlockSourceFromMainChunkSource(),
                            pos->getBlockPos() + BlockPos(0, 1, 0), mirror,
                            rotation);
