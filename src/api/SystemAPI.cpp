  ...
  try {
    ...
    return Boolean::newBoolean(NewProcess(
        ...
        [callback{std::move(callbackFunc)},
         engine{EngineScope::currentEngine()}](int exitCode, string output) {
          ll::schedule::DelayTask<ll::chrono::ServerClock>(
              ll::chrono::ticks(1), [engine, callback = std::move(callback),
                                     exitCode, output = std::move(output)]() {
                ...
                try {
                  NewTimeout(
                      callback.get(),
                      {Number::newNumber(exitCode), String::newString(output)},
                      1);
                }
    ...
    return Boolean::newBoolean(NewProcess(
        ...
        [callback{std::move(callbackFunc)},
         engine{EngineScope::currentEngine()}](int exitCode, string output) {
          ll::schedule::DelayTask<ll::chrono::ServerClock>(
              ll::chrono::ticks(1),
              [engine, callback = std::move(callback), exitCode,
