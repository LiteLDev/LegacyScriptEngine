...
template <> Local<Value> any_to(const Any &val) {
  switch (val.type) {
  case Any::Type::Null:
    return Local<Value>();
...
// 生成函数
DBSessionClass::DBSessionClass(const Local<Object> &scriptObj,
...
DBSessionClass *DBSessionClass::constructor(const Arguments &args) {
  try {
    ...
    case 1: {
      ...
      if (args[0].isString()) {
        result = new DBSessionClass(args.thiz(),
                                    ConnParams(args[0].asString().toString()));
...
Local<Value> DBSessionClass::prepare(const Arguments &args) {
  ...
  try {
    auto stmt =
        new DBStmtClass(session->prepare(args[0].asString().toString()));
...
// 生成函数
DBStmtClass::DBStmtClass(const Local<Object> &scriptObj,
