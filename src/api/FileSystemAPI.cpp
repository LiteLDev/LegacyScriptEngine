...
ClassDefine<FileClass> FileClassBuilder =
    defineClass<FileClass>("File")
        ...
        .property("ReadMode",
                  [] { return Number::newNumber((int)FileOpenMode::ReadMode); })
        .property(
            "WriteMode",
            [] { return Number::newNumber((int)FileOpenMode::WriteMode); })
        .property(
            "AppendMode",
            [] { return Number::newNumber((int)FileOpenMode::AppendMode); })
...
// 生成函数
FileClass::FileClass(const Local<Object> &scriptObj, std::fstream &&f,
                     const std::string &path, bool isBinary)
...
FileClass *FileClass::constructor(const Arguments &args) {
  ...
  try {
    ...
    LOG_ERROR_WITH_SCRIPT_INFO("Fail to Open File " +
...
Local<Value> FileClass::readAllSync(const Arguments &args) {
  try {
    string res((std::istreambuf_iterator<char>(file)),
               std::istreambuf_iterator<char>());
    return isBinary
               ? ByteBuffer::newByteBuffer(res.data(), res.size()).asValue()
...
Local<Value> FileClass::writeSync(const Arguments &args) {
  ...
  try {
    if (args[0].isString()) {
      ...
      file.write((char *)args[0].asByteBuffer().getRawBytes(),
...
Local<Value> FileClass::readLine(const Arguments &args) {
  ...
  try {
    ...
    pool.enqueue([fp{&file}, lock{&lock}, callback{std::move(callbackFunc)},
...
Local<Value> FileClass::readAll(const Arguments &args) {
  ...
  try {
    ...
    pool.enqueue([fp{&file}, isBinary{isBinary}, lock{&lock},
      ...
      lock->lock();
      string res((std::istreambuf_iterator<char>(*fp)),
                 std::istreambuf_iterator<char>());
      lock->unlock();
...
Local<Value> FileClass::write(const Arguments &args) {
  ...
  try {
    ...
    if (args[0].isString()) {
      ...
      isString = false;
      data = std::move(string((char *)args[0].asByteBuffer().getRawBytes(),
                              args[0].asByteBuffer().byteLength()));
    } else {
      LOG_WRONG_ARG_TYPE();
      return {};
...
Local<Value> DirCreate(const Arguments &args) {
  ...
  try {
    ...
    LOG_ERROR_WITH_SCRIPT_INFO("Fail to Create Dir " +
                               args[0].asString().toString() + "!\n");
    return Boolean::newBoolean(false);
  }
...
Local<Value> PathDelete(const Arguments &args) {
  ...
  try {
    return Boolean::newBoolean(remove_all(ll::string_utils::str2wstr(
                                   args[0].asString().toString())) > 0);
  } catch (const filesystem_error &e) {
    LOG_ERROR_WITH_SCRIPT_INFO("Fail to Delete " +
                               args[0].asString().toString() + "!\n");
    return Boolean::newBoolean(false);
  }
...
Local<Value> PathCopy(const Arguments &args) {
  ...
  try {
    copy(ll::string_utils::str2wstr(args[0].asString().toString()),
...
Local<Value> PathRename(const Arguments &args) {
  ...
  try {
    rename(ll::string_utils::str2wstr(args[0].asString().toString()),
           ll::string_utils::str2wstr(args[1].asString().toString()));
    return Boolean::newBoolean(true);
  } catch (const filesystem_error &e) {
    LOG_ERROR_WITH_SCRIPT_INFO("Fail to Rename " +
                               args[0].asString().toString() + "!\n");
    return Boolean::newBoolean(false);
  }
...
Local<Value> CheckIsDir(const Arguments &args) {
  ...
  try {
    ...
    LOG_ERROR_WITH_SCRIPT_INFO("Fail to Get Type of " +
                               args[0].asString().toString() + "!\n");
    return {};
  }
...
Local<Value> GetFileSize(const Arguments &args) {
  ...
  try {
    ...
    LOG_ERROR_WITH_SCRIPT_INFO("Fail to Get Size of " +
                               args[0].asString().toString() + "!\n");
    return {};
  }
  CATCH("Fail in GetFilesList!");
...
Local<Object> FileClass::newFile(std::fstream &&f, const std::string &path,
                                 bool isBinary) {
