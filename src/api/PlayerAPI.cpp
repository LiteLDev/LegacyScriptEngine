...
ClassDefine<PlayerClass> PlayerClassBuilder =
    defineClass<PlayerClass>("LLSE_Player")
        ...
        .instanceProperty("canStartSleepInBed",
                          &PlayerClass::getCanStartSleepInBed)
        ...
        .instanceProperty("isTouchingDamageBlock",
                          &PlayerClass::isTouchingDamageBlock)
        ...
        .instanceFunction("setMaxAttackDamage",
                          &PlayerClass::setMaxAttackDamage)
        .instanceFunction("setFollowRange", &PlayerClass::setFollowRange)
        .instanceFunction("setKnockbackResistance",
                          &PlayerClass::setKnockbackResistance)
        .instanceFunction("setLuck", &PlayerClass::setLuck)
        .instanceFunction("setMovementSpeed", &PlayerClass::setMovementSpeed)
        .instanceFunction("setUnderwaterMovementSpeed",
                          &PlayerClass::setUnderwaterMovementSpeed)
        .instanceFunction("setLavaMovementSpeed",
                          &PlayerClass::setLavaMovementSpeed)
        ...
        .instanceFunction("getBlockStandingOn",
                          &PlayerClass::getBlockStandingOn)
        ...
        .instanceFunction("getRespawnPosition",
                          &PlayerClass::getRespawnPosition)
        .instanceFunction("setRespawnPosition",
                          &PlayerClass::setRespawnPosition)
        ...
        .instanceFunction("getCurrentExperience",
                          &PlayerClass::getCurrentExperience)
        .instanceFunction("setCurrentExperience",
                          &PlayerClass::setCurrentExperience)
        .instanceFunction("getTotalExperience",
                          &PlayerClass::getTotalExperience)
        .instanceFunction("setTotalExperience",
                          &PlayerClass::setTotalExperience)
        .instanceFunction("getXpNeededForNextLevel",
                          &PlayerClass::getXpNeededForNextLevel)
        ...
        .instanceFunction("getEntityFromViewVector",
                          &PlayerClass::getEntityFromViewVector)
        .instanceFunction("getBlockFromViewVector",
                          &PlayerClass::getBlockFromViewVector)
        .instanceFunction("quickEvalMolangScript",
                          &PlayerClass::quickEvalMolangScript)
        ...
        .instanceFunction("simulateDisconnect",
                          &PlayerClass::simulateDisconnect)
        ...
        .instanceFunction("simulateSetBodyRotation",
                          &PlayerClass::simulateSetBodyRotation)
        .instanceFunction("simulateNavigateTo",
                          &PlayerClass::simulateNavigateTo)
        .instanceFunction("simulateUseItem", &PlayerClass::simulateUseItem)
        .instanceFunction("simulateStopDestroyingBlock",
                          &PlayerClass::simulateStopDestroyingBlock)
        .instanceFunction("simulateStopInteracting",
                          &PlayerClass::simulateStopInteracting)
        .instanceFunction("simulateStopMoving",
                          &PlayerClass::simulateStopMoving)
        .instanceFunction("simulateStopUsingItem",
                          &PlayerClass::simulateStopUsingItem)
        .instanceFunction("simulateStopSneaking",
                          &PlayerClass::simulateStopSneaking)
...
Player *PlayerClass::extract(Local<Value> v) {
  if (EngineScope::currentEngine()->isInstanceOf<PlayerClass>(v))
    return EngineScope::currentEngine()
        ->getNativeInstance<PlayerClass>(v)
        ->get();
  else
    return nullptr;
}
...
Local<Value> McClass::setPlayerScore(const Arguments &args) {
  ...
  try {
    ...
    Objective *objective =
        scoreboard.getObjective(args[1].asString().toString());
    if (!objective) {
      return Boolean::newBoolean(false);
    }
    const ScoreboardId &id = scoreboard.getScoreboardId(
        PlayerScoreboardId(std::atoll(args[0].asString().toString().c_str())));
    if (!id.isValid()) {
      Player *pl = ll::service::getLevel()->getPlayer(uuid);
      if (pl) {
    ...
    scoreboard.modifyPlayerScore(isSuccess, id, *objective,
...
Local<Value> McClass::deletePlayerScore(const Arguments &args) {
  ...
  try {
    ...
    return Boolean::newBoolean(
        scoreboard.getScoreboardIdentityRef(id)->removeFromObjective(
...
Local<Value> McClass::getPlayer(const Arguments &args) {
  ...
  try {
    ...
    for (Player *p : playerList) {
      if (p->getXuid() == target ||
          std::to_string(p->getOrCreateUniqueID().id) == target)
    ...
    return found ? PlayerClass::newPlayer(found)
                 : Local<Value>(); // Player/Null
  }
...
Local<Value> PlayerClass::getPermLevel() {
  try {
    ...
    return Number::newNumber(
        magic_enum::enum_integer(player->getCommandPermissionLevel()));
  }
  ...
  try {
    ...
    if (!player)
      ...
      return String::newString(
          std::to_string(player->getOrCreateUniqueID().id));
  }
  ...
  try {
    ...
    if (newPerm >= 0 && newPerm <= 4) {
      RecordOperation(ENGINE_OWN_DATA()->pluginName, "Set Permission Level",
                      fmt::format("Set Player {} Permission Level as {}.",
                                  player->getRealName(), newPerm));
      player->setPermissions((CommandPermissionLevel)newPerm);
      if (newPerm >= 1) {
        player->getAbilities().setPlayerPermissions(
            PlayerPermissionLevel::Operator);
      } else {
        player->getAbilities().setPlayerPermissions(
            PlayerPermissionLevel::Member);
      }
      UpdateAbilitiesPacket uPkt(player->getOrCreateUniqueID(),
                                 player->getAbilities());
...
Local<Value> PlayerClass::setRespawnPosition(const Arguments &args) {
  try {
    ...
    if (args.size() == 1) {
      ...
      CHECK_ARG_TYPE(args[2], ValueKind::kNumber);
      CHECK_ARG_TYPE(args[3], ValueKind::kNumber);
      pos = {args[0].toInt(), args[1].toInt(), args[2].toInt(),
             args[3].toInt()};
    } else {
...
Local<Value> PlayerClass::getBlockStandingOn(const Arguments &args) {
  try {
    ...
    return BlockClass::newBlock(player->getBlockPosCurrentlyStandingOn(nullptr),
...
Local<Value> PlayerClass::getScore(const Arguments &args) {
  ...
  try {
    ...
    const ScoreboardId &id =
        scoreboard.getScoreboardId(player->getOrCreateUniqueID());
    if (!id.isValid()) {
...
Local<Value> PlayerClass::setScore(const Arguments &args) {
  ...
  try {
    ...
    bool isSuccess = false;
    scoreboard.modifyPlayerScore(isSuccess, id, *obj,
...
Local<Value> PlayerClass::deleteScore(const Arguments &args) {
  ...
  try {
    ...
    return Boolean::newBoolean(
        scoreboard.getScoreboardIdentityRef(id)->removeFromObjective(scoreboard,
                                                                     *obj));
  }
...
Local<Value> PlayerClass::setExtraData(const Arguments &args) {
  ...
  try {
    ...
    ENGINE_OWN_DATA()->playerDataDB[player->getRealName() + "-" + key] =
        args[1];
    return Boolean::newBoolean(true);
  }
...
Local<Value> PlayerClass::setHungry(const Arguments &args) {
  ...
  try {
    ...
    AttributeInstance *healthAttribute =
        player->getMutableAttribute(player->HUNGER);
...
Local<Value> PlayerClass::getMoney(const Arguments &args) {
  try {
    ...
    return xuid.empty() ? Local<Value>()
                        : Number::newNumber(EconomySystem::getMoney(xuid));
  }
...
Local<Value> PlayerClass::reduceMoney(const Arguments &args) {
  ...
  try {
    ...
    auto xuid = player->getXuid();
    return xuid.empty() ? Local<Value>()
                        : Boolean::newBoolean(EconomySystem::reduceMoney(
...
Local<Value> PlayerClass::setMoney(const Arguments &args) {
  ...
  try {
    ...
    auto xuid = player->getXuid();
    return xuid.empty() ? Local<Value>()
                        : Boolean::newBoolean(EconomySystem::setMoney(
                              xuid, args[0].asNumber().toInt64()));
  }
...
Local<Value> PlayerClass::addMoney(const Arguments &args) {
  ...
  try {
    ...
    auto xuid = player->getXuid();
    return xuid.empty() ? Local<Value>()
                        : Boolean::newBoolean(EconomySystem::addMoney(
                              xuid, args[0].asNumber().toInt64()));
  }
...
Local<Value> PlayerClass::transMoney(const Arguments &args) {
  ...
  try {
    ...
    return xuid.empty()
               ? Local<Value>()
               : Boolean::newBoolean(EconomySystem::transMoney(
                     xuid, targetXuid, args[0].asNumber().toInt64(), note));
  }
...
Local<Value> PlayerClass::getMoneyHistory(const Arguments &args) {
  ...
  try {
    ...
    return xuid.empty()
               ? Local<Value>()
               : objectificationMoneyHistory(
                     EconomySystem::getMoneyHist(xuid, args[0].toInt()));
  }
      ...
      if (IsInstanceOf<IntPos>(args[0])) {
        ...
        if (posObj->dim < 0)
          ...
          pos = *posObj;
                 ...
                 IsInstanceOf<EntityClass>(args[0])) {
        // Player or Entity
...
Local<Value> PlayerClass::addEffect(const Arguments &args) {
  ...
  try {
    ...
    MobEffectInstance effect =
        MobEffectInstance(id, tick, level, false, showParticles, false);
