...
struct PyConfig;
typedef PyObject *(*create_stdio_func_type)(const PyConfig *config,
                                            PyObject *io, int fd,
...
bool loadPythonPlugin(std::string dirPath, const std::string &packagePath,
  ...
  if (dirPath == LLSE_PLUGIN_PACKAGE_TEMP_DIR) {
    ...
    if (std::filesystem::exists(LLSE_PLUGIN_PACKAGE_TEMP_DIR
                                "/pyproject.toml")) {
      auto pluginName =
          PythonHelper::getPluginPackageName(LLSE_PLUGIN_PACKAGE_TEMP_DIR);
      if (pluginName.empty()) {
        pluginName = UTF82String(filesystem::path(packagePath)
                                     .filename()
                                     .replace_extension("")
                                     .u8string());
      }
      auto dest =
          std::filesystem::path(LLSE_PLUGINS_ROOT_DIR).append(pluginName);
  ...
  auto realPackageInstallDir =
      (filesystem::path(dirPath) / "site-packages").make_preferred();
  if (!filesystem::exists(realPackageInstallDir)) {
    std::string dependTmpFilePath =
        PythonHelper::getPluginPackDependencyFilePath(dirPath);
  ...
  try {
    ...
    try {
      ...
      string pluginSitePackageFormatted = UTF82String(
          std::filesystem::canonical(realPackageInstallDir.make_preferred(), ec)
              .u8string());
      if (!ec) {
        engine->eval("_llse_py_sys_module.path.insert(0, r'" +
                     pluginSitePackageFormatted + "')");
      }
      // add plugin source dir to sys.path
      string sourceDirFormatted = UTF82String(
          std::filesystem::canonical(filesystem::path(dirPath).make_preferred())
              .u8string());
      engine->eval("_llse_py_sys_module.path.insert(0, r'" +
                   sourceDirFormatted + "')");
      ...
      string entryPathFormatted =
          UTF82String(std::filesystem::canonical(
                          filesystem::path(entryPath).make_preferred())
                          .u8string());
    ...
    if (!PluginManager::getPlugin(pluginName)) {
      ...
      try {
        std::filesystem::path packageFilePath =
            std::filesystem::path(dirPath) / "pyproject.toml";
        string packageFilePathStr =
            UTF82String(packageFilePath.make_preferred().u8string());
        ...
        if (projectNode["version"]) {
          ver = ll::Version::parse(
              *(projectNode["version"].value<std::string>()));
        }
      ...
      // register
      PluginManager::registerPlugin(dirPath, pluginName, description, ver,
                                    others);
    }
    ...
    logger.info(tr("llse.loader.loadMain.loadedPlugin",
                   fmt::arg("type", "Python"), fmt::arg("name", pluginName)));
    return true;
  } catch (const Exception &e) {
...
std::string getPluginPackageName(const std::string &dirPath) {
  auto dirPath_obj = std::filesystem::path(dirPath);
  std::string defaultReturnName =
      UTF82String(filesystem::path(dirPath).filename().u8string());

  std::filesystem::path packageFilePath =
      dirPath_obj / std::filesystem::path("pyproject.toml");
  ...
  try {
    ...
    std::optional<std::string> packageName =
        projectNode["name"].value<std::string>();
...
std::string getPluginPackDependencyFilePath(const std::string &dirPath) {
  ...
  std::filesystem::path packageFilePath =
      dirPath_obj / std::filesystem::path("pyproject.toml");
  std::filesystem::path requirementsFilePath =
      dirPath_obj / std::filesystem::path("requirements.txt");
  ...
  if (std::filesystem::exists(requirementsFilePath)) {
    std::error_code ec;
    std::filesystem::copy_file(requirementsFilePath, requirementsTmpFilePath,
                               ec);
  }
  ...
  if (std::filesystem::exists(packageFilePath)) {
    ...
    if (!dependsAdded.empty()) {
      std::ofstream fout(
          UTF82String(requirementsTmpFilePath.make_preferred().u8string()),
          std::ios::app);
      fout << dependsAdded;
      fout.close();
    }
  ...
  if (isInConsoleDebugMode) {
    ...
    if (cmd == "stop") {
      ...
      try {
        ...
        PyRun_StringFlags(codeBuffer.c_str(), Py_single_input,
                          getPyGlobalDict(), nullptr, nullptr);
        codeBuffer.clear();
...
PyObject *create_stdio_hooked(const PyConfig *config, PyObject *io, int fd,
                              int write_mode, const char *name,
                              const wchar_t *encoding, const wchar_t *errors) {
  if (fd == 0) {
    Py_RETURN_NONE;
  }
  return create_stdio_original(config, io, fd, write_mode, name, encoding,
                               errors);
}
...
bool patchPython310CreateStdio() {
  if (create_stdio_original == nullptr) {
    ...
    create_stdio_original =
        (create_stdio_func_type)(void *)(((uintptr_t)hModule) +
  ...
  if (DetourTransactionBegin() != NO_ERROR)
    ...
    return false;
  else if (DetourAttach((PVOID *)&create_stdio_original, create_stdio_hooked) !=
           NO_ERROR)
    return false;
...
bool unpatchPython310CreateStdio() {
  if (DetourTransactionBegin() != NO_ERROR)
    ...
    return false;
  else if (DetourDetach((PVOID *)&create_stdio_original, create_stdio_hooked) !=
           NO_ERROR)
    return false;
  else if (DetourTransactionCommit() != NO_ERROR)
    return false;
