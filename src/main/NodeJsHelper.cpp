...
std::unordered_map<script::ScriptEngine *,
                   std::unique_ptr<node::CommonEnvironmentSetup>> *setups =
    new std::unordered_map<script::ScriptEngine *,
                           std::unique_ptr<node::CommonEnvironmentSetup>>();
...
bool initNodeJs() {
  ...
  using namespace v8;
  platform =
      node::MultiIsolatePlatform::Create(std::thread::hardware_concurrency());
...
script::ScriptEngine *newEngine() {
  ...
  std::unique_ptr<node::CommonEnvironmentSetup> setup =
      node::CommonEnvironmentSetup::Create(
          platform.get(), &errors, args, exec_args,
  ...
  script::ScriptEngine *engine =
      new script::ScriptEngineImpl({}, isolate, setup->context(), false);
  ...
  try {
    ...
    string executeJs =
        ...
        *mainScripts + "\n})({}, __LLSE_PublicRequire, __LLSE_PublicModule, '" +
        entryScriptPath + "', '" + pluginDirPath +
        "'); "; // TODO __filename & __dirname need to be reviewed
    // TODO: ESM Support
    ...
    // Load code
    MaybeLocal<v8::Value> loadenv_ret =
...
bool loadNodeJsPlugin(std::string dirPath, const std::string &packagePath,
                      bool isHotLoad) {
  ...
  if (dirPath == LLSE_PLUGIN_PACKAGE_TEMP_DIR) {
    ...
    if (std::filesystem::exists(LLSE_PLUGIN_PACKAGE_TEMP_DIR "/package.json")) {
      auto pluginName =
          NodeJsHelper::getPluginPackageName(LLSE_PLUGIN_PACKAGE_TEMP_DIR);
      if (pluginName.empty()) {
  ...
  if (NodeJsHelper::doesPluginPackHasDependency(dirPath) &&
    ...
    if ((exitCode = NodeJsHelper::executeNpmCommand("npm install", dirPath)) ==
        0)
      logger.info(tr("llse.loader.nodejs.executeNpmInstall.success"));
    else
      logger.error(tr("llse.loader.nodejs.executeNpmInstall.fail",
  ...
  try {
    ...
    if (!PluginManager::getPlugin(pluginName)) {
      ...
      try {
        std::filesystem::path packageFilePath =
            std::filesystem::path(dirPath) / "package.json";
        std::ifstream file(
        ...
        } else if (j["repository"].is_object() &&
                   j["repository"].contains("url")) {
          others["Repository"] = j["repository"]["url"].get<std::string>();
        }
      } catch (...) {
        logger.warn(tr("llse.loader.nodejs.register.fail",
    ...
    logger.info(tr("llse.loader.loadMain.loadedPlugin",
                   fmt::arg("type", "Node.js"), fmt::arg("name", pluginName)));
    return true;
...
std::string findEntryScript(const std::string &dirPath) {
  ...
  try {
    ...
    auto entryPath = std::filesystem::canonical(
        dirPath_obj / std::filesystem::path(entryFile));
...
std::string getPluginPackageName(const std::string &dirPath) {
  ...
  std::filesystem::path packageFilePath =
      dirPath_obj / std::filesystem::path("package.json");
  if (!std::filesystem::exists(packageFilePath))
    return "";
...
int executeNpmCommand(std::string cmd, std::string workingDir) {
...

  {
    ...
    try {
      node::SetProcessExitHandler(env, [&](node::Environment *env_,
                                           int exit_code) { node::Stop(env); });
      MaybeLocal<v8::Value> loadenv_ret =
          node::LoadEnvironment(env, executeJs.c_str());
      if (loadenv_ret.IsEmpty()) // There has been a JS exception.
        throw "error";
