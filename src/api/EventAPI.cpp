...
Local<Value> McClass::listen(const Arguments &args) {
  ...
  try {
    return Boolean::newBoolean(LLSEAddEventListener(
        EngineScope::currentEngine(), args[0].toStr(), args[1].asFunction()));
  }
...
bool LLSEAddEventListener(ScriptEngine *engine, const string &eventName,
...
bool LLSERemoveAllEventListeners(ScriptEngine *engine) {
  for (auto &listeners : listenerList) {
    listeners.remove_if(
        [engine](auto &listener) { return listener.engine == engine; });
  }
  return true;
}
...
void EnableEventListener(int eventId) {
  ...
  case EVENT_TYPES::onPreJoin:
    Event::PlayerPreJoinEvent::subscribe([](const PlayerPreJoinEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onPreJoin) {
        CallEvent(EVENT_TYPES::onPreJoin,
                  PlayerClass::newPlayer((Player *)ev.mPlayer));
      }
  ...
  case EVENT_TYPES::onChat:
    Event::PlayerChatEvent::subscribe([](const PlayerChatEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onChat) {
        CallEvent(EVENT_TYPES::onChat, PlayerClass::newPlayer(ev.mPlayer),
                  String::newString(ev.mMessage));
      }
    ...
    Event::PlayerAttackBlockEvent::subscribe(
        [](const PlayerAttackBlockEvent &ev) {
          IF_LISTENED(EVENT_TYPES::onAttackBlock) {
            CallEvent(
                EVENT_TYPES::onAttackBlock, PlayerClass::newPlayer(ev.mPlayer),
                BlockClass::newBlock(ev.mBlockInstance),
                !ev.mItemStack->isNull() ? ItemClass::newItem(ev.mItemStack)
                                         : Local<Value>());
          }
    ...
    Event::PlayerUseItemOnEvent::subscribe([](const PlayerUseItemOnEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onUseItemOn) {
        CallEvent(EVENT_TYPES::onUseItemOn,
                  ...
                  FloatPos::newPos(ev.mClickPos, ev.mPlayer->getDimensionId()));
      }
      ...
      if (ev.mEventType == PlayerUseBucketEvent::EventType::Place) {
        IF_LISTENED(EVENT_TYPES::onUseBucketPlace) {
          CallEvent(
              ...
              BlockClass::newBlock(ev.mBlockInstance),
              Number::newNumber(ev.mFace),
              FloatPos::newPos(ev.mTargetPos, ev.mPlayer->getDimensionId()));
        }
  ...
  case EVENT_TYPES::onSetArmor:
    Event::PlayerSetArmorEvent::subscribe([](const PlayerSetArmorEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onSetArmor) {
        CallEvent(EVENT_TYPES::onSetArmor, PlayerClass::newPlayer(ev.mPlayer),
                  Number::newNumber(ev.mSlot),
  ...
  case EVENT_TYPES::onEffectUpdated:
    Event::PlayerEffectChangedEvent::subscribe([](const PlayerEffectChangedEvent
                                                      &ev) {
      if (ev.mEventType == PlayerEffectChangedEvent::EventType::Add) {
        IF_LISTENED(EVENT_TYPES::onEffectAdded) {
          CallEvent(
              EVENT_TYPES::onEffectAdded, PlayerClass::newPlayer(ev.mPlayer),
        ...
        IF_LISTENED(EVENT_TYPES::onEffectRemoved) {
          CallEvent(
              EVENT_TYPES::onEffectRemoved, PlayerClass::newPlayer(ev.mPlayer),
              String::newString(ev.mEffect->getComponentName().getString()));
        }
        ...
        IF_LISTENED(EVENT_TYPES::onEffectUpdated) {
          CallEvent(
              EVENT_TYPES::onEffectUpdated, PlayerClass::newPlayer(ev.mPlayer),
              String::newString(ev.mEffect->getComponentName().getString()),
              Number::newNumber(ev.mEffect->getAmplifier()),
              Number::newNumber(ev.mEffect->getDuration()));
        }
    ...
    Event::CmdBlockExecuteEvent::subscribe([](const CmdBlockExecuteEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onCmdBlockExecute) {
        if (ev.mIsMinecart) {
          ...
          BlockInstance bl = ev.mBlockInstance;
          CallEvent(
              EVENT_TYPES::onCmdBlockExecute, String::newString(ev.mCommand),
              FloatPos::newPos(bl.getPosition().toVec3(), bl.getDimensionId()),
              Boolean::newBoolean(ev.mIsMinecart));
        }
      }
  ...
  case EVENT_TYPES::onMobTrySpawn:
    Event::MobTrySpawnEvent::subscribe([](const MobTrySpawnEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onMobTrySpawn) {
        CallEvent(EVENT_TYPES::onMobTrySpawn, String::newString(ev.mTypeName),
                  FloatPos::newPos(ev.mPos, ev.mDimensionId));
      }
  ...
  case EVENT_TYPES::onBedEnter:
    Event::PlayerBedEnterEvent::subscribe([](const PlayerBedEnterEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onBedEnter) {
        BlockInstance bl(*ev.mBlockInstance);
        CallEvent(EVENT_TYPES::onBedEnter, PlayerClass::newPlayer(ev.mPlayer),
                  IntPos::newPos(bl.getPosition(), bl.getDimensionId()));
      }
  ...
  case EVENT_TYPES::onAttack:
    Event::PlayerAttackEvent::subscribe([](const PlayerAttackEvent &ev) {
      IF_LISTENED(EVENT_TYPES::onAttack) {
        if (ev.mTarget) {
          CallEvent(EVENT_TYPES::onAttack, PlayerClass::newPlayer(ev.mPlayer),
                    EntityClass::newEntity(ev.mTarget));
        }
      }
...
bool MoneyBeforeEventCallback(LLMoneyEvent type, xuid_t from, xuid_t to,
                              money_t value) {
  switch (type) {
  case LLMoneyEvent::Add: {
    IF_LISTENED(EVENT_TYPES::beforeMoneyAdd) {
      CallEvent(EVENT_TYPES::beforeMoneyAdd, String::newString(to),
  ...
  case LLMoneyEvent::Reduce: {
    IF_LISTENED(EVENT_TYPES::beforeMoneyReduce) {
      CallEvent(EVENT_TYPES::beforeMoneyReduce, String::newString(to),
                Number::newNumber(value));
    }
  ...
  case LLMoneyEvent::Trans: {
    IF_LISTENED(EVENT_TYPES::beforeMoneyTrans) {
      CallEvent(EVENT_TYPES::beforeMoneyTrans, String::newString(from),
                String::newString(to), Number::newNumber(value));
    }
  ...
  case LLMoneyEvent::Set: {
    IF_LISTENED(EVENT_TYPES::beforeMoneySet) {
      CallEvent(EVENT_TYPES::beforeMoneySet, String::newString(to),
                Number::newNumber(value));
    }
...
bool MoneyEventCallback(LLMoneyEvent type, xuid_t from, xuid_t to,
                        money_t value) {
  switch (type) {
  case LLMoneyEvent::Add: {
    IF_LISTENED(EVENT_TYPES::onMoneyAdd) {
      CallEvent(EVENT_TYPES::onMoneyAdd, String::newString(to),
                Number::newNumber(value));
    }
  ...
  case LLMoneyEvent::Reduce: {
    IF_LISTENED(EVENT_TYPES::onMoneyReduce) {
      CallEvent(EVENT_TYPES::onMoneyReduce, String::newString(to),
                Number::newNumber(value));
    }
  ...
  case LLMoneyEvent::Trans: {
    IF_LISTENED(EVENT_TYPES::onMoneyTrans) {
      CallEvent(EVENT_TYPES::onMoneyTrans, String::newString(from),
                String::newString(to), Number::newNumber(value));
    }
  ...
  case LLMoneyEvent::Set: {
    IF_LISTENED(EVENT_TYPES::onMoneySet) {
      CallEvent(EVENT_TYPES::onMoneySet, String::newString(to),
                Number::newNumber(value));
    }
