...
// 实例类类型检查
template <typename T> bool inline IsInstanceOf(Local<Value> v) {
...
// 参数类型错误输出
#define LOG_WRONG_ARG_TYPE()                                                   \
  LOG_ERROR_WITH_SCRIPT_INFO("Wrong type of argument!");

// 参数数量错误输出
#define LOG_TOO_FEW_ARGS() LOG_ERROR_WITH_SCRIPT_INFO("Too Few arguments!");

// 参数数量错误输出
#define LOG_WRONG_ARGS_COUNT()                                                 \
  LOG_ERROR_WITH_SCRIPT_INFO("Wrong number of arguments!");
...
#define CHECK_ARGS_COUNT(count)                                                \
  if (args.size() != count) {                                                  \
    throw Exception(                                                           \
        fmt::format("Invalid arguments count: {}, expect {}, in API {}",       \
                    args.size(), count, __FUNCTION__));                        \
  }
...
#define CHECK_ARG_TYPE(index, type)                                            \
  if (args[index].getKind() != ValueKind::type) {                              \
    throw Exception(                                                           \
        fmt::format("Wrong type of arguments[{}]: {}, expect {}, in API {}",   \
                    index, ValueKindToString(args[index].getKind()),           \
...
#define CHECK_VAL_TYPE(val, type)                                              \
  if (val.getKind() != ValueKind::type) {                                      \
    throw Exception(                                                           \
        fmt::format("Wrong type of value: {}, expect {}, in API {}",           \
...
template <typename Type> struct EnumDefineBuilder {
  template <Type val> inline static Local<Value> serialize() {
  ...
  inline static Local<Value> getName(const Arguments &args) {
    try {
      if (args.size() < 1)
        return Local<Value>();
      if (args[0].isString())
        return magic_enum::enum_cast<Type>(args[0].toStr()).has_value()
                   ? args[0]
                   : Local<Value>();
      if (args[0].isNumber())
        return String::newString(
            magic_enum::enum_name(static_cast<Type>(args[0].toInt())));
      return Local<Value>();
    } catch (const std::exception &) {
      logger.error("Error in " __FUNCTION__);
    }
  ...
  template <Type val,
            std::enable_if_t<std::is_enum_v<Type>, char> max =
  ...
  template <std::enable_if_t<std::is_enum_v<Type>, char> max =
                static_cast<char>(*magic_enum::enum_values<Type>().rbegin())>
