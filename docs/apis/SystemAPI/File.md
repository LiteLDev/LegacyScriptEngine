...
Before 0.19.0, this function could only be used as `File.readFrom(path)`, and the return value type was always `String`.
...
!!! warning
Passing `ByteBuffer` is only available in 0.19.0 and later.
