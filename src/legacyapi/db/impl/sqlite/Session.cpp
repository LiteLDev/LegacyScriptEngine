...
SQLiteSession::~SQLiteSession() {
  IF_ENDBG dbLogger.debug("SQLiteSession::~SQLiteSession: Destructor: this: {}",
                          (void *)this);
  close();
}
...
void SQLiteSession::open(const ConnParams &params) {
  ...
  auto flags = 0;
  if (p.get<bool>({"create", "create_if_not_exist", "createifnotexist"}, true,
                  false)) {
    flags |= SQLITE_OPEN_CREATE;
  }
  if (p.get<bool>({"readonly", "readonly_mode", "readonlymode", "r"}, false,
                  false)) {
    flags |= SQLITE_OPEN_READONLY;
  }
  if (p.get<bool>(
          {"readwrite", "readwrite_mode", "readwritemode", "read_write", "rw"},
          false, false)) {
  ...
  if (res != SQLITE_OK) {
    throw std::runtime_error("SQLiteSession::open: Failed to open database: " +
                             std::string(sqlite3_errmsg(conn)));
  }
  IF_ENDBG dbLogger.debug("SQLiteSession::open: Opened database: " +
                          std::string(path));
}
...
Session &SQLiteSession::query(const std::string &query,
  ...
  if (conn) {
    ...
    if (res != SQLITE_OK) {
      throw std::runtime_error(
          "SQLiteSession::close: Failed to close database: " +
          std::string(sqlite3_errmsg(conn)));
    }
    conn = nullptr;
