...
SimpleIni *SimpleIni::create(const std::string &path,
                             const std::string &defContent) {
  if (!std::filesystem::exists(ll::string_utils::str2wstr(path))) {
    // 创建新的
    std::filesystem::create_directories(
        std::filesystem::path(ll::string_utils::str2wstr(path))
            .remove_filename()
            .u8string());
...
bool SimpleIni::setString(const string &sec, const string &key,
                          const string &value) {
...
string SimpleIni::getString(const string &sec, const string &key,
                            const string &def) {
