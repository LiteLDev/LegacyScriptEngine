...
void FormatHelper(std::vector<Local<Value>> &args,
                  const std::vector<char *> &names, bool enableObject,
...
Local<Value> I18nClass::load(const Arguments &args) {
  ...
  try {
    ...
    if (path.ends_with('/') || path.ends_with('\\') ||
      ...
      res =
          new ll::i18n::MultiFileI18N(path, defaultLocaleName, defaultLangData);
    } else {
      res = new ll::i18n::SingleFileI18N(path, defaultLocaleName,
                                         defaultLangData);
    }
