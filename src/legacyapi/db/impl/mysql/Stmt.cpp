...
template <> MYSQL_TIME any_to(const DB::Any &v) {
  switch (v.type) {
  ...
  while (reader.isValid()) {
    ...
    if (c == '\'') {
      ...
      if (result.count(name)) {
        throw std::runtime_error("ParseStmtParams: Duplicate parameter name: " +
                                 name);
      }
      result[name] = cur++;
      // Remove the name so that mysql can parse it
      query = query.substr(0, reader.getPos() - name.length() - delta) +
              query.substr(reader.getPos() - delta);
...
AllocateBuffer(const MYSQL_FIELD &field) {
  std::size_t len = 0;
  switch (field.type) {
  ...
  case MYSQL_TYPE_NEWDECIMAL:
    ...
    break;
  default:
    throw std::runtime_error("AllocateBuffer: Unsupported MySQL data type: " +
...
Any ReceiverToAny(const Receiver &rec) {
  ...
  case MYSQL_TYPE_JSON:
#if defined(LLDB_DEBUG_MODE)
    dbLogger.debug("ReceiverToAny: string: length: {}, buffer {}", rec.length,
                   rec.buffer.get());
#endif
  ...
  case MYSQL_TYPE_LONG_BLOB:
#if defined(LLDB_DEBUG_MODE)
    dbLogger.debug("ReceiverToAny: blob: length: {}, buffer {}", rec.length,
                   rec.buffer.get());
#endif
  ...
  case MYSQL_TYPE_NEWDECIMAL:
    // TODO: Decimal
    dbLogger.debug("MySQL_Decimal: {}",
                   std::string(rec.buffer.get(), rec.length));
    return Any();
  default:
    throw std::runtime_error("ReceiverToAny: Unsupported MySQL data type: " +
                             std::to_string(rec.field.type));
  }
}
...
int MySQLStmt::getNextParamIndex() {
  ...
  IF_ENDBG dbLogger.debug(
      "MySQLStmt::getNextParamIndex: The next param index is {}", result + 1);
  return result + 1;
}
...
void MySQLStmt::bindResult() {
  ...
  if (mysql_stmt_errno(stmt) && !metadata) {
    throw std::runtime_error(
        "MySQLStmt::bindResult: Failed to get result metadata" +
...
Stmt &MySQLStmt::bind(const Any &value, int index) {
  ...
  if (getUnboundParams() == 0) {
    throw std::runtime_error(
        "MySQLStmt::bind: All the parameters are already bound");
  }
  ...
  case Any::Type::String: {
    ...
    IF_ENDBG dbLogger.debug("MySQLStmt::bind: Bound string param at {}: {}",
                            index, param.buffer.get());
    break;
  }
  case Any::Type::Date:
  case Any::Type::Time:
  ...
  if (getUnboundParams() == 0) {
    ...
    if (res) {
      throw std::runtime_error("MySQLStmt::bind: " +
                               std::string(mysql_stmt_error(stmt)));
    }
    if (autoExecute)
      execute();
...
Stmt &MySQLStmt::bind(const Any &value, const std::string &name) {
  if (!paramIndexes.count(name)) {
    throw std::invalid_argument("MySQLStmt::bind: Parameter name `" + name +
                                "` not found");
  }
...
Stmt &MySQLStmt::execute() {
  ...
  if (res) {
    ...
    throw std::runtime_error("MySQLStmt::execute: Failed to execute stmt: " +
                             std::string(mysql_stmt_error(stmt)));
  }
...
bool MySQLStmt::step() {
  ...
  if (res == MYSQL_NO_DATA) {
    ...
    throw std::runtime_error(
        fmt::format("MySQLStmt::step: {}", mysql_stmt_error(stmt)));
  }
  ++steps;
  return true;
}
...
Row MySQLStmt::_Fetch() {
  ...
  IF_ENDBG dbLogger.debug("MySQLStmt::_Fetch: RowHeader size {}",
                          row.header->size());
  int i = 0;
...
Stmt &MySQLStmt::reset() {
  if (mysql_stmt_reset(stmt)) {
    throw std::runtime_error("MySQLStmt::reset: " +
                             std::string(mysql_stmt_error(stmt)));
  }
  result.reset();
  resultHeader.reset();
...
SharedPointer<Stmt> MySQLStmt::create(const std::weak_ptr<Session> &session,
  ...
  if (raw->debugOutput && !params.empty()) {
    ...
    dbLogger.debug("MySQLStmt::create: - SQL without named parameters: {}",
                   query);
    for (auto &[k, v] : params) {
  ...
  if (res) {
    throw std::runtime_error("MySQLStmt::create: " +
                             std::string(mysql_stmt_error(stmt)));
  }
