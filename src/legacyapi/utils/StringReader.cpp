...
StringReader::StringReader(const std::string &str)
    : str(str), len(str.length()), begin(str.begin()), end(str.end()),
      it(begin) {}
StringReader::StringReader(const char *cstr)
    : str(cstr), len(str.length()), begin(str.begin()), end(str.end()),
      it(begin) {}
...
char StringReader::read() {
  if (isEnd()) {
    throw std::out_of_range(
        "StringReader::read: The end of the string has been reached");
  }
  return *it++;
}
...
std::string StringReader::readLetters(const std::string &chars) {
  std::string result;
  while (!isEnd() &&
         (isalpha(peek()) || chars.find(peek()) != std::string::npos)) {
    result += read();
  }
  return result;
}
...
std::string StringReader::readLower(const std::string &chars) {
  std::string result;
  while (!isEnd() &&
         (islower(peek()) || chars.find(peek()) != std::string::npos)) {
    result += read();
  }
  return result;
}
...
std::string StringReader::readUpper(const std::string &chars) {
  std::string result;
  while (!isEnd() &&
         (isupper(peek()) || chars.find(peek()) != std::string::npos)) {
    result += read();
  }
  return result;
}
...
std::string StringReader::readDigits(const std::string &chars) {
  std::string result;
  while (!isEnd() &&
         (isdigit(peek()) || chars.find(peek()) != std::string::npos)) {
    result += read();
  }
  return result;
}
...
std::string StringReader::readLettersAndDigits(const std::string &chars) {
  std::string result;
  while (!isEnd() &&
         (isalnum(peek()) || chars.find(peek()) != std::string::npos)) {
    result += read();
  }
  return result;
}
...
char StringReader::peek() {
  if (isEnd()) {
    throw std::out_of_range(
        "StringReader::peek: The end of the string has been reached");
  }
  return *it;
}
...
char StringReader::peek(size_t offset) {
  ...
  if (offset >= getRemaining()) {
    throw std::out_of_range(
        "StringReader::peek: The offset is greater than the remaining length");
  }
  return *(it + offset);
}
...
void StringReader::skip() {
  if (isEnd()) {
    throw std::out_of_range(
        "StringReader::skip: The end of the string has been reached");
  }
  ++it;
}
...
void StringReader::skip(size_t len) {
  ...
  if (len > getRemaining()) {
    throw std::out_of_range(
        "StringReader::skip: The length is greater than the remaining length");
  }
  it += len;
}
