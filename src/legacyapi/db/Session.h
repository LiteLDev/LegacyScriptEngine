...
protected:
  ...
  std::vector<std::weak_ptr<Stmt>>
      stmtPool; ///< List of statements opened by prepare method.
...
public:
   ...
   *  None
   */
  virtual bool relogin(const std::string &user, const std::string &password,
                       const std::string &db = "");
  /**
   ...
   */
  virtual Session &query(const std::string &query,
                         std::function<bool(const Row &)> callback) = 0;
  /**
   * @brief Execute a query.
   *
   ...
   * @endcode
   */
  virtual SharedPointer<Stmt> prepare(const std::string &query,
                                      bool autoExecute = false) = 0;
  /**
   ...
   * @return SharedPointer<Session>  The session
   */
  LIAPI static SharedPointer<Session>
  create(DBType type, const std::string &host, uint16_t port,
...
private:
   ...
   * @return SharedPointer<Session>  The session
   */
  static SharedPointer<Session> _Create(DBType type,
                                        const ConnParams &params = {});

private:
  static std::vector<std::weak_ptr<Session>>
      sessionPool; ///< List of sessions(weak pointers)
...
public:
  ...
  static std::shared_ptr<Session> getSession(Session *session) {
    ...
    throw std::runtime_error(
        "Session::getSession: Session is not found or expired");
  }
};
