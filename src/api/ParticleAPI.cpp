...
#define GETVEC3(v, d, u)                                                       \
  if (IsInstanceOf<IntPos>(u)) {                                               \
    ...
    auto pos2 =                                                                \
        EngineScope::currentEngine() -> getNativeInstance<FloatPos>(u);        \
    v = Vec3(pos2->x, pos2->y, pos2->z);                                       \
    d = pos2->dim;                                                             \
  } else {                                                                     \
    LOG_WRONG_ARG_TYPE();                                                      \
...
namespace {
template <typename T> std::string fto_string(const T a_value) {
...
Local<Value> McClass::newParticleSpawner(const Arguments &args) {
  ...
  return EngineScope::currentEngine()->newNativeClass<ParticleSpawner>(
      displayRadius, highDetial, doubleSide);
}
...
Local<Value> ParticleSpawner::drawPoint(const Arguments &args) {
  ...
  if (args.size() > 1) {
    ...
    pointSize =
        static_cast<ParticleCUI::PointSize>(args[1].asNumber().toInt32());
  }
  std::string color = "W";
  if (args.size() > 2) {
...
Local<Value> ParticleSpawner::drawNumber(const Arguments &args) {
  ...
  ParticleCUI::spawnParticle(pos, std::string("ll:num") + numName + color,
                             dimId);
  return Boolean::newBoolean(true);
}
Local<Value> ParticleSpawner::drawAxialLine(const Arguments &args) {
  ...
  ParticleCUI::drawAxialLine(pos, direction, length, dimId,
                             getColorType(color));
  return Boolean::newBoolean(true);
}
Local<Value> ParticleSpawner::drawOrientedLine(const Arguments &args) {
  ...
  if (size > 2) {
    CHECK_ARG_TYPE(args[2], ValueKind::kNumber);
    lineWidth =
        static_cast<ParticleCUI::PointSize>(args[2].asNumber().toInt32());
  }
  ...
  ParticleCUI::drawOrientedLine(start, end, dimId1, lineWidth, minSpacing,
                                maxParticlesNum, getColorType(color));
...
Local<Value> ParticleSpawner::drawCuboid(const Arguments &args) {
  ...
  if (IsInstanceOf<IntPos>(args[0])) {
    auto pos2 =
        EngineScope::currentEngine()->getNativeInstance<IntPos>(args[0]);
    min = Vec3(pos2->x, pos2->y, pos2->z);
    dimId1 = pos2->dim;
  } else if (IsInstanceOf<FloatPos>(args[0])) {
    auto pos2 =
        EngineScope::currentEngine()->getNativeInstance<FloatPos>(args[0]);
    min = Vec3(pos2->x, pos2->y, pos2->z);
    dimId1 = pos2->dim;
  } else {
    LOG_WRONG_ARG_TYPE();
  ...
  if (size > 1) {
    if (IsInstanceOf<IntPos>(args[1])) {
      auto pos2 =
          EngineScope::currentEngine()->getNativeInstance<IntPos>(args[1]);
      max = Vec3(pos2->x, pos2->y, pos2->z) + 1;
      dimId2 = pos2->dim;
    } else if (IsInstanceOf<FloatPos>(args[1])) {
      auto pos2 =
          EngineScope::currentEngine()->getNativeInstance<FloatPos>(args[1]);
...
Local<Value> ParticleSpawner::drawCircle(const Arguments &args) {
  ...
  if (size > 3) {
    CHECK_ARG_TYPE(args[3], ValueKind::kNumber);
    lineWidth =
        static_cast<ParticleCUI::PointSize>(args[3].asNumber().toInt32());
  }
  ...
  ParticleCUI::drawCircle(pos, facing, radius, dimId, lineWidth, minSpacing,
                          maxParticlesNum, getColorType(color));
...
ClassDefine<ParticleSpawner> ParticleSpawnerBuilder =
    defineClass<ParticleSpawner>("ParticleSpawner")
        ...
        .instanceProperty("displayRadius", &ParticleSpawner::getDisplayRadius,
                          &ParticleSpawner::setDisplayRadius)
        .instanceProperty("highDetial", &ParticleSpawner::getHighDetial,
                          &ParticleSpawner::setHighDetial)
        .instanceProperty("doubleSide", &ParticleSpawner::getDoubleSide,
                          &ParticleSpawner::setDoubleSide)
        ...
        .instanceFunction("drawOrientedLine",
