...
void InitGlobalShareData() {
    ...
    if (hGlobalData == NULL) {
        lse::LegacyScriptEngine::getInstance().getSelf().getLogger().error("init.fileMapping.fail"_tr());
    ...
    if (address == NULL) {
        lse::LegacyScriptEngine::getInstance().getSelf().getLogger().error("init.mapFile.fail"_tr());
        localShareData->isFirstInstance = true;
        return;
    }
