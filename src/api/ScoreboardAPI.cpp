...
Local<Value> ObjectiveClass::setDisplay(const Arguments &args) {
  ...
  try {
    ...
    return Boolean::newBoolean(
        ll::service::getLevel()->getScoreboard().setDisplayObjective(
            slot, *obj, (ObjectiveSortOrder)sort));
  }
...
Local<Value> ObjectiveClass::setScore(const Arguments &args) {
  ...
  try {
    ...
    if (args[0].isString()) {
      ...
      scoreboard.modifyPlayerScore(isSuccess, id, *obj, score,
...
Local<Value> ObjectiveClass::deleteScore(const Arguments &args) {
  ...
  try {
    if (args[0].isString()) {
      ...
      const ScoreboardId &id =
          scoreboard.getScoreboardId(args[0].asString().toString());
      if (!id.isValid()) {
...
Local<Value> ObjectiveClass::getScore(const Arguments &args) {
  ...
  try {
    if (args[0].isString()) {
      ...
      ScoreboardId sid = board.getScoreboardId(PlayerScoreboardId(
          std::atoll(args[0].asString().toString().c_str())));
...
Local<Value> McClass::getDisplayObjective(const Arguments &args) {
  ...
  try {
    ...
    auto res =
        ll::service::getLevel()->getScoreboard().getDisplayObjective(slot);
...
Local<Value> McClass::clearDisplayObjective(const Arguments &args) {
  ...
  try {
    string slot = args[0].toStr();
    auto res =
        ll::service::getLevel()->getScoreboard().clearDisplayObjective(slot);
...
Local<Value> McClass::newScoreObjective(const Arguments &args) {
  ...
  try {
    ...
    Objective *obj = scoreboard.addObjective(
        name, display,
        *const_cast<ObjectiveCriteria *>(scoreboard.getCriteria(criteria)));
